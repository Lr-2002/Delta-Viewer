...
根据产品负责人明确指令，正式版从 `1.0.1` 开始编号，基于已发布 `0.17.67` 的截取恢复、完成状态和 NAS 回放优化。机标校对仅进入独立开发版，正式版不包含校对入口或机标读取。版本编号调整不改变当前 UNSIGNED 分发方式，也不代表 GAP-003/GAP-007 实机及大容量验收已经完成。正式应用保留 `com.dohc.viewer` 标识及已有账号配置、草稿和本机标注路径，以便覆盖升级。
