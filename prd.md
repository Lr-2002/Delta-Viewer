...
本文同时记录当前 `0.12.0` Alpha 已经验证的能力和正式发布前仍需完成的工作。标记为“已实现”不代表已经通过 Windows 发布验收；Windows 安装包、真实 SD 卡和长时数据测试仍是独立的发布门槛。
...
- 不提供多段拼接、多轨编辑、数据标注或训练任务管理；v0.9 只提供单条轨迹的连续时间范围裁剪。
...
1. 操作员插入 exFAT SD 卡并选择卡根目录，应用扫描记录并自动选择第一条 session。
2. 应用直接请求本地目标目录，随后复制数据并验证目标端文件；不需要再次点击导入按钮。
3. 应用自动运行全量结构/状态检查和固定百分位 JPEG 抽检，并给出通过、警告或失败状态。
4. 用户在五路画面和状态曲线之间同步定位异常帧。
5. 用户在回放页设置起止帧，播放只覆盖选中片段并查看片段状态数/时长。
6. 用户选择目标格式和目录，生成选中片段的数据集并获得输出路径和统计信息。
...
物理 SD 卡
   |
   v
选择目录 -> 自动扫描 episode -> 自动选择首条记录 -> 选择本地目录 -> 复制到本地
                                                                    |
                                                                    v
                                                        大小/BLAKE3 回读校验
                                                                    |
...
| FR-VAL-010 | P0 | 提供汇总与逐问题视图。 | 展示状态、已检查文件数、耗时、各流总帧、实际抽检/检查帧数、解码失败数和 issue code。 | 已实现 |
| FR-VAL-011 | P0 | error 在 UI 和 Rust 导出入口形成双重阻断。 | 不能通过直接调用 IPC 绕过检查。 | 已实现并测试 |
| FR-VAL-012 | P1 | 导出机器可读检查报告。 | 可生成包含版本、图像检查模式/百分位、issue 和统计的 JSON 文件。 | 已实现并测试 |
...
机器可读报告使用 `formatVersion=2`，包含 `episodeRoot`、`parsedStateCount`、`imageValidationMode`（`sampled` 或 `full`）、`imageSamplePercentages`、文件/流统计和完整 issue 列表。每个流的 `checkedFrames` 是实际解码数；抽检报告固定记录 `[1,25,50,73,99]`，全量报告记录空数组。可定位的 issue 附带可选 `frameId`。报告先写入隐藏 partial 文件并回读验证，再原子发布，同名时不覆盖。抽检报告不代表未抽中 JPEG 已通过解码检查。
...
- 数据集 metadata 包含源名称、状态条数和 `clip_start_frame`/`clip_end_frame`。
...
- 顶栏：产品名、当前源路径、健康状态和选择 SD 卡；选择后自动继续扫描、首条 session 导入与检查，不提供单独导入按钮。
...
- 每个流显示总帧、实际抽检/检查帧数、解码失败和结果；抽检列提示固定百分位。
- Issue 列表显示 scope、中文消息和稳定 code。

### 10.4 导出页

- 显示源记录概况、选中帧范围/状态数/时长、三种格式单选、当前 adapter 和导出命令。
...
- macOS 当前用于优先开发和 unsigned debug 验证，不改变 Windows 10/11 x64 的首个正式发布目标。
- 可分发 app/DMG 必须使用只依赖 macOS 系统库的 reviewed FFmpeg；Homebrew 动态链接版本只能标记为 local-debug。
- 无头构建可以生成不含 Finder 窗口美化的内容等价 DMG，并必须只读挂载验证 app、`/Applications` 链接、版本和 FFmpeg 资源 hash。
- unsigned debug app/DMG 不得对外发布；正式 macOS 发布仍需确定最低系统版本、Developer ID 签名、notarization 和目标机器测试。
