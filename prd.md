...
| 产品名称 | DOHC Viewer |
| 文档版本 | 0.35 |
| 应用版本基线 | 1.0.2-dev.1（开发版）；1.0.1（正式版） |
...
本分支为独立开发版 `1.0.2-dev.1`，使用 `DOHC Viewer Dev` 名称和 `com.dohc.viewer.dev` 本机目录，不安装正式更新通道的资产。机标对照位于独立“校对”工作区，仅点击该页时读取机标文件；人工回放标注页不展示机标。正式版按用户明确指令从 `1.0.1` 编号，基于 `0.17.67` 的已发布优化，不包含机标功能。两个版本均保留当前 UNSIGNED 分发语义和既有实机验收缺口。
