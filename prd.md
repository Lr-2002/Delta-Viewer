...
| 文档版本 | 0.17 |
| 应用版本基线 | 0.17.0 |
| 文档状态 | 安全 Alpha，五安装器 unsigned GitHub Release CD 与平台完整性门禁已定义，等待可信签名与目标机验收 |
| 发布平台 | Windows 10/11 x64；macOS 12+ arm64/x64；Ubuntu 22.04+ x86_64 deb；Ubuntu 20.04+ x86_64 Flatpak 兼容包 |
| 文档日期 | 2026-07-22 |
| 产品负责人 | 待指定 |
| 技术负责人 | 待指定 |
...
本文同时记录当前 `0.17.0` Alpha 已经验证的能力和正式发布前仍需完成的工作。标记为“已实现”不代表已经通过目标机发布验收；当前 GitHub Release 明确为没有可信发布者身份的 unsigned 通道，可信签名安装包、真实 SD 卡和长时数据测试仍是独立的生产门槛。
...
| D-003 | 正式安装包覆盖 Windows 10/11 x64、macOS 12+ arm64/x64 与 Ubuntu 20.04+ x86_64；首个现场验收重点仍为 Windows。技术栈为 Tauri 2、Rust、React 和 TypeScript。 |
...
| D-015 | v0.12 起选择 SD 卡目录后自动扫描、选择并处理第一条 session，不再要求点击“导入并检查”；v0.16 的全部 session 自动导入与管理工作区由 D-023 取代旧目标目录交互。 |
...
| D-019 | Release CD 只从 clean exact annotated tag 构建，并且必须同时完成 Windows x64、macOS arm64、macOS x64、Ubuntu 22.04+ x64 deb 和 Ubuntu 20.04+ x64 Flatpak 五个安装包；当前阶段允许公开显式 `UNSIGNED` 的完整集合，但标题、说明、文件名、报告和 manifest 必须一致披露。未来签名产物必须使用新版本/tag，不覆盖 unsigned 资产。 |
...
| D-024 | v0.16 起 Ubuntu 20.04+ x86_64 通过 GNOME 50 Flatpak 正式分发；Ubuntu 24.04 生成的 `.deb` 只作为 Flatpak 输入。Flatpak 仅开放显示、DRI、IPC 及 `/media`、`/run/media`、`/mnt`，不得开放 network。Ubuntu 可原生只读挂载 ext4 SD 卡。 |
| D-025 | v0.17 起 Ubuntu 22.04+ x86_64 首选原生 `.deb`，由 Ubuntu 22.04 runner 构建并通过 `apt` 真实安装、依赖/资源回读和 Xvfb 启动检查；GNOME 50 Flatpak 继续作为 Ubuntu 20.04+ 兼容入口。两份 Linux 安装器必须同时进入 Release。 |
...
- Ubuntu 22.04+ x86_64 目标机可通过原生 deb 安装；Ubuntu 20.04+ x86_64 可通过 Flatpak 兼容包安装，并从已挂载的 ext4/exFAT SD 卡完成同一离线工作流。
- 现场用户无需命令行脚本即可完成主要任务。
...
- 不提供断点续传；一次选卡会自动排队导入扫描到的全部 episode，但不会并行复制多个大文件任务。
...
3. 应用自动创建本机工作区，依次复制全部发现的 session 并验证目标端文件；不请求用户选择目标目录，也不需要再次点击导入按钮。
4. 首个成功导入的 session 自动运行结构/状态检查和固定百分位 JPEG 抽检；其余 session 完成导入后可从左侧进入检查。
...
物理 SD 卡
   |
   v
本地注册/登录 -> 选择目录 -> 自动扫描全部 episode -> 应用工作区自动复制全部记录
                                                                    |
                                                                    v
                                                        大小/BLAKE3 回读校验
                                                                    |
...
| 已扫描 | 已发现 episode，尚未复制 | 全部记录自动进入导入队列 |
...
| FR-SRC-008 | P0 | 选择 SD 卡目录后自动加载全部 session。 | 扫描成功后应用自动创建 app-local-data 工作区并依次导入全部发现的 session；首个成功项自动检查并回放；界面没有本地目标选择框或额外“导入并检查”按钮。 | 已实现 |

### 8.2 本地导入与完整性

| 编号 | 优先级 | 需求 | 验收标准 | 状态 |
| --- | --- | --- | --- | --- |
| FR-IMP-001 | P0 | 选择 SD 卡后由应用自动选择当前用户可写的本地工作区并开始复制。 | 不出现本地目标目录对话框；工作区位于 app-local-data/imports 下，源卡不产生任何写入。 | 已实现 |
...
| FR-VIS-011 | P1 | 连续播放不显示逐帧加载文案。 | 播放过程中不出现“解码中”覆盖层；暂停、拖动或步进等待帧时保留加载提示，不可用帧仍显示明确错误。 | 已实现 |
...
- 源路径只读；导入写入只发生在应用管理的 app-local-data/imports 工作区，导出写入发生在用户选择的导出目录。
...
- 正式安装包支持 Windows 10/11 x64、macOS 12+ arm64、macOS 12+ x64、Ubuntu 22.04+ x86_64 deb，以及 Ubuntu 20.04+ x86_64 Flatpak 兼容包。
...
- 文件选择仅由原生目录对话框触发；导入目标不向用户暴露选择框。
- Tauri capability 只开放核心窗口和目录对话框需要的权限。
- Windows/macOS/Linux 发布依赖必须锁定版本并完成许可证审查；Ubuntu Flatpak runtime 固定 GNOME 50，构建阶段访问依赖和签名服务不改变安装后核心功能的离线边界。
...
- 顶栏：产品名、当前源路径、健康状态、选择 SD 卡、操作错误历史、当前登录账号和退出按钮；选择后自动继续扫描、全部 session 导入，首个成功项进入检查与回放，不提供单独导入按钮。
...
### 12.3 Ubuntu deb 与 Flatpak 包要求

- 原生 deb 正式支持 x86_64 Ubuntu 22.04 及以上，文件名固定为 `DOHC-Viewer_<version>_UNSIGNED_ubuntu-22.04+-x64.deb`；Ubuntu 20.04+ 继续通过 `DOHC-Viewer_<version>_UNSIGNED_ubuntu-x64.flatpak` 获得兼容支持。
- Linux job 固定在 Ubuntu 22.04 runner 构建。deb 必须声明 `libwebkit2gtk-4.1-0`、`libgtk-3-0`、`libayatana-appindicator3-1` 和 `librsvg2-2`，并用 `apt` 安装后检查 `amd64` 元数据、动态库、应用资源和启动；不得把只解包或只构建当作安装通过。
- Flatpak 由同一份已经验证的 deb 和 `packaging/flatpak/com.dohc.viewer.json` 重打包，继续隔离宿主 WebKitGTK/glibc 差异。
- Flatpak 使用固定 `org.gnome.Platform//50` 与 `org.gnome.Sdk//50`，command 为 `dohc-viewer`，App ID 为 `com.dohc.viewer`。
- `finish-args` 只允许 Wayland、fallback X11、DRI、IPC 以及 `/media`、`/run/media`、`/mnt` 的读写权限；不得增加 `--share=network` 或其他网络 socket。
- bundle 必须包含 Tauri 主程序、FFmpeg、合并许可证、provenance manifest、desktop 文件和 AppStream metainfo；安装后回读每项 hash，并在 Xvfb + D-Bus 下启动 smoke 10 秒。
- Flatpak 用户通过 Flathub 安装 GNOME runtime；原生 deb 不依赖 Flatpak/Flathub。应用自身不联网、不写源 SD 卡，源卡必须由系统以只读方式挂载。
...
- Release 标题、说明、五个 installer 名称、verification report 和 manifest 必须显示 `UNSIGNED`；后续引入签名时恢复 Authenticode、timestamp、Developer ID、Gatekeeper、notarization 和可信 Linux 包发布门禁。
- final job 重新核对五份 verification report 和安装器 SHA-256，生成 `release-manifest.json`、`SHA256SUMS.txt` 和 GitHub provenance。五安装器集合完整后才解除 draft，已经公开的 tag 不允许覆盖。
...
- unsigned debug bundle 只证明本机构建和资源布局；unsigned GitHub Release 还必须通过五安装器 CI 安装或挂载、直接启动、依赖和完整集合门禁，但两者都不能替代生产签名和目标机器验收。
...
| 编号 | 场景 | 预期结果 |
| --- | --- | --- |
| AT-001 | 选择包含多个标准 episode 的 SD 卡根目录 | 正确发现全部记录并显示五路、状态和容量，自动创建 app-local-data 工作区并导入全部 session；首个成功项自动检查/回放，不弹本地目标选择框 |
| AT-001a | 导入过程中系统返回 `Operation not permitted`/`operation not allowed` | 顶部显示权限提示，原始消息以 `PERMISSION_DENIED` 记录到本地错误历史；重新登录后仍可回读记录 |
...
| AT-027 | 推送 clean exact annotated release tag | CD 同时生成文件名带 `UNSIGNED` 的 Windows x64 NSIS、macOS arm64/x64 DMG、Ubuntu 22.04+ x86_64 deb 和 Ubuntu 20.04+ x86_64 Flatpak；五者均披露无可信发布者身份并通过依赖、安装或挂载、启动和 hash 检查；macOS 还通过 strict ad-hoc seal 与 synthetic-quarantine Gatekeeper 分类，Ubuntu 两个包均通过 Xvfb 启动检查后才公开 Release |
...
| AT-030 | 在 Ubuntu 22.04 CI 安装并运行 Ubuntu Flatpak | runtime 为 GNOME 50、App ID/desktop/metainfo 正确，仅有 SD 卡路径权限且无 network；FFmpeg/许可证/manifest hash 回读一致，应用在 Xvfb + D-Bus 中保持运行 10 秒 |
...
- macOS app/DMG、Windows NSIS、Linux deb/Flatpak 的 FFmpeg 二进制、许可证及 provenance manifest 资源配置。
...
- 五安装器正式 Release CD、完整集合发布门禁、安装器 verification report、SHA-256 manifest 和 GitHub build provenance 工作流。
...
| GAP-010 | Ubuntu deb/Flatpak 已有 CI 安装和 Xvfb smoke，但尚无 Ubuntu 22.04 deb、Ubuntu 20.04 Flatpak 实机、真实 ext4 SD 卡和长时导入证据 | 在 Ubuntu 22.04+ x86_64 实机验收 deb，在 Ubuntu 20.04+ x86_64 实机验收 Flatpak，并挂载真实 SD 卡完成只读导入、检查、回放、三格式导出和权限错误历史验收 |
...
- Flatpak 从同一份已验证 deb 构建并继续执行 runtime、权限、资源和启动检查。final job 必须同时收到 deb 与 Flatpak 的独立报告，五个安装器全部匹配后才公开 Release。
...
| M1 多平台 Alpha | Windows x64 离线安装包、macOS arm64/x64 DMG、Ubuntu 22.04+ x86_64 deb、Ubuntu 20.04+ x86_64 Flatpak、reviewed FFmpeg 和基本硬件测试 | 进行中；五安装器 unsigned CD 与完整性门禁已定义，目标机待验收 |
