...
1.0.44 两个新增相机槽位分别固定绑定 `extension_left`、`extension_right`，标题保留目录名称，缺少任一路时保留同名占位；继续兼容其他动态发现的流。各路使用对应源的 JPEG/manifest 播放路径，分辨率从已加载图像/视频元数据补齐，不额外扫描全部媒体。所有相机在窄屏仍显示分辨率，长名称可换行；切换 session 后不沿用上条数据的尺寸。
