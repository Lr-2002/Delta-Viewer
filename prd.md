...
本文同时记录当前 `0.1.0` 原型已经验证的能力和正式发布前仍需完成的工作。标记为“已实现”不代表已经通过 Windows 发布验收；Windows 安装包、真实 SD 卡和长时数据测试仍是独立的发布门槛。
...
| FR-SRC-005 | P0 | 源目录在整个应用流程中只读。 | 自动化测试和代码审查确认没有对源路径执行写入、重命名或删除。 | 发布门槛 |
| FR-SRC-006 | P1 | 明确显示卷类型、可移动介质状态和可用容量。 | UI 可以区分 SD 卡与普通本地目录。 | 待实现 |
...
| FR-IMP-004 | P0 | 复制后重新打开目标文件，校验大小和 BLAKE3。 | 任一不一致立即失败，不发布最终目录。 | 已实现 |
...
| FR-IMP-009 | P0 | 导入前检查目标剩余空间和文件系统能力。 | 空间不足或不支持大文件时，在复制前阻止任务。 | 待实现 |
| FR-IMP-010 | P1 | 取消/失败的 partial 目录可被识别和清理。 | 下次启动可提示清理，不会把 partial 当作 episode。 | 待实现 |

数据集 BLAKE3 的输入序列定义为：对按相对路径排序的每个文件依次写入 UTF-8 相对路径、单个 `0x00`、小端 `u64` 文件大小、该文件 BLAKE3 的 ASCII 十六进制文本。
...
| FR-VAL-011 | P0 | error 在 UI 和 Rust 导出入口形成双重阻断。 | 不能通过直接调用 IPC 绕过检查。 | UI 已实现，后端待实现 |
| FR-VAL-012 | P1 | 导出机器可读检查报告。 | 可生成包含版本、issue 和统计的 JSON 文件。 | 待实现 |
...
| FR-VIS-007 | P1 | 播放速率根据记录 FPS 而非固定 30 FPS。 | 从状态时间戳估计并允许用户覆盖。 | 待实现 |
| FR-VIS-008 | P1 | 支持按 issue 跳转到相关帧。 | 点击解码、缺帧或时间问题可定位时间轴。 | 待实现 |
...
| FR-EXP-002 | P0 | warning 可以导出，error 必须阻止。 | warning 明确显示；发布版要求用户确认。 | 部分实现 |
...
| FR-EXP-007 | P1 | 导出完成后可在资源管理器中打开位置。 | 一次点击打开输出父目录并选中结果。 | 待实现 |
...
| AT-002 | 导入标准样例 | 981 个文件、80,531,730 字节校验通过，生成 manifest 和稳定数据集 BLAKE3 |
...
- 本地复制、逐文件 BLAKE3、目标回读和 manifest。
...
| GAP-004 | 导入前没有空间和 FAT32 检查 | 复制前明确阻断空间不足和不支持大文件的目标 |
| GAP-005 | Rust 导出入口未强制复跑/验证 error gate | IPC 直接调用也无法导出 error 数据 |
| GAP-006 | warning 导出缺少显式确认 | 用户确认后才进入 adapter |
| GAP-007 | 长时/大容量数据性能未知 | 完成至少 100 GB/100,000 文件压力测试并记录基线 |
| GAP-008 | partial 清理 UX 未实现 | 启动时可识别并安全清理本应用生成的 partial |
