...
| 产品名称 | DOHC Viewer |
| 文档版本 | 0.35 |
| 应用版本基线 | 1.0.2-dev.2（开发版）；1.0.1（正式版） |
...
2026-09-08 补充：开发版校对布局按用户参考图提供左侧主视频、起止帧及属性，右侧分段条和动作列表。用户明确要求优先发布可安装的 macOS Apple Silicon 开发 DMG，允许该预发行版先交付 Mac；正式版仍遵守三平台完整发布门禁。开发版不进入正式版自动更新。

本分支为独立开发版 `1.0.2-dev.2`，使用 `DOHC Viewer Dev` 名称和 `com.dohc.viewer.dev` 本机目录，不安装正式更新通道的资产。机标对照位于独立“校对”工作区，仅点击该页时读取机标文件；人工回放标注页不展示机标。正式版按用户明确指令从 `1.0.1` 编号，基于 `0.17.67` 的已发布优化，不包含机标功能。两个版本均保留当前 UNSIGNED 分发语义和既有实机验收缺口。

开发版自 `1.0.2-dev.2` 起按用户要求恢复可读取视频的标注员人工标注流程：`TRAJECTORY_STATIC` 和 `TRAJECTORY_POSITION_UNAVAILABLE` 作为轨迹警告显示，用户确认“仍要标注”后恢复任务标注、裁剪与片段保存。该开发版规则覆盖下文 FR-VAL-015 的自动跳过行为；不可用图像帧门禁和正式版行为保持原样，校对功能不变。
...
机标对照新增范围：读取每条源数据根目录的 `bailian_annotation.json`（schema v3），以目录名匹配 `episode_results[].episode_id`。机标是独立只读参考，不修改源文件、不进入人工修订或“已标注”统计，也不上传用户中心。回放显示机标动作、帧区间、模型与校验状态，并对照当前帧的已保存人工片段。定位/播放机标使用独立临时范围，结束后返回原人工范围；预览时禁用人工保存。Camera 0 媒体帧通过源流帧数和主时间轴范围映射，禁止使用状态总数拉伸机标。显式遵守源文件区间与帧基数；缺少声明的历史 v3 显示 0 起始、左闭右开假设，无法匹配源视频时禁用定位。机标缺失、非法、超限或读取失败不阻断视频与人工标注，加载按源身份丢弃过期响应。机标解析限 8 MiB、2000 片段，鉴权后的 Rust blocking worker 读取固定文件，拒绝符号链接。
