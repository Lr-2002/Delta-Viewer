...
| D-017 | 所有数据用户进入工作区前必须登录当前主机局域网用户中心账号；登录页允许自助注册普通标注员，管理员只能在服务主机初始化或由现有管理员创建，离线身份模式停用。已登录标注员可以更新自己的当前显示名称，稳定账号名和角色不可自助修改；监管端刷新后按稳定账号聚合并展示新姓名。服务保存 scrypt 密码哈希、账号字段和 D-032 的标注绩效审计事件；客户端通过管理员导入的固定 HTTPS 证书配置注册或登录，登录 token 只在 Rust 进程内保存。 |
...
| D-035 | 监管端按任务数量分配不重叠的 episode 序号区间；用户中心不得接收 NAS 路径或 episode ID。每台工作电脑仅在本机保存一次已挂载 NAS 根目录，普通账号后续登录后自动扫描并只展示其序号区间内的视频条目；本地盘、可移动卷和网络挂载统一执行当前账号任务过滤，不允许借数据源类型绕过。 |
...
| D-036 | 读取流程同时识别 `h264-split-mp4-v1` 记录：根级 `manifest.json` 声明五路固定流的分段 MP4、帧率、帧数和分辨率，`states.jsonl` 使用 60 Hz `batch_id` 主时间轴并允许位姿位于可空的 `pose` 对象中。界面按主时间轴和各流清单帧率只读同步解码预览，不修改或复制 MP4；每条清单 MP4 均通过各自的进程内只读媒体地址交给 WebView 持续解码，该服务仅绑定 `127.0.0.1` 随机端口并支持单区间 Range。该通道不是远程数据源协议，只注册后端已规范化并确认位于当前 episode 内的普通 MP4，以每次进程随机的不透明 token 寻址，不返回文件系统路径、不监听局域网、不写缓存文件。Camera 0 保持主时钟与起播门禁，其他原生播放器独立加载并在首次定位或进入可播放状态后自动补启动；不存在 MP4 或原生通道失败的流保留逐帧兼容回退。连续播放不得对任何可用的清单 MP4 逐帧启动 FFmpeg。隐藏的 recorder benchmark/QC 目录不作为 episode。首版 MP4 兼容范围仅为扫描、状态读取和回放；仍依赖逐帧 JPEG 语义的 MCAP/HDF5/LeRobot adapter 必须后端明确阻断，直至各格式完成 MP4 原生适配与回读验收。 |
| D-037 | 监管工作台保留原任务完成概览，从已扫描任务目录和已导入任务详情生成统一任务池；具体分配工作区仅在管理员点击某个操作员的分配入口后出现，并可显式关闭返回概览。管理员按操作员勾选具体任务，并为每类任务填写正整数数量；有 NAS 目录统计时，新勾选任务和当前搜索结果默认使用该任务文件夹全部数量，仍可手动减少且不得超过目录总量；仅导入任务 JSON 时默认数量为 1。批量分配提供两个明确模式：“整文件夹”允许多选已扫描任务目录并以各目录全部视频数一次分配完，“按任务数量”允许多选任务并分别填写每个任务的分配总量；两种模式均支持多选标注员、平均或历史速度策略，结果不得与未选标注员的已有数量合计超过目录总量。截止字段使用本地日期时间并精确到分钟，管理员和标注员界面显示完整截止时间。服务持久化任务名到数量的精确映射、优先级、顺序、状态和截止时间，总分配量为各项之和；同一任务已分配给其他账号时，保存前必须列出冲突账号并由管理员确认是否继续。保存结果或错误必须紧邻分配控件显示并立即刷新概览。用户中心必须在健康响应中声明结构化任务分配能力，客户端在读取和保存前强制校验，并验证保存回包完整保留任务名、数量和总数，禁止旧服务产生假成功。界面展示任务完成量、剩余量、预计完成时间、文件夹总量、详情和重复分配提示，支持搜索、拖拽排序、暂停、追加、减少、单项或批量转移；旧账号的数字分配值不在新界面展示，首次保存具体任务后转为结构化分配。普通账号在账户旁的任务抽屉查看本机任务路径、分配数量、起始序号和按日期过滤的本人审计记录，并可在本机选择任务根目录后加载匹配目录；路径不得上传用户中心。 |
| D-038 | 读取流程同时识别直接包含 `segment-<十进制编号>.bin` 的 T265 目录。segment 数量不固定，按编号数值排序后组成一条连续 episode；无关文件不参与统计、指纹或解析，编号缺口产生 warning，重复编号/帧号和损坏记录产生 error。`DHSG`/`DHSC` v1 小端记录中的左右目 JPEG 与 pose 复用固定 `t265_left`、`t265_right` 和状态模型，`batch_id` 为共同帧号，header 的 `capture_time_ns` 为状态时间。结构扫描读取全部记录头尾和 pose，图像按索引偏移按需读取并校验 payload/整记录 CRC32；源文件保持只读。当前兼容范围为扫描、检查与回放，依赖独立 JPEG 文件路径的 MCAP/HDF5/LeRobot adapter 由后端明确阻断。 |
...
提供绑定当前账号和已加载 episode 的本机草稿。草稿默认以完整轨迹创建片段 1，用户可在当前帧分割所在片段，形成覆盖完整轨迹且互不重叠的连续闭区间，并可编辑
片段名称和注解、点击时间线跳转。切换 episode、刷新或网络中断后可恢复，不写源卡；正式保存成功后清理草稿，并由
...
- 不提供云账号、组织级角色权限、跨机器标注同步或忘记密码服务；当前主机局域网用户中心仅提供普通标注员自助注册、账号登录和管理员账号管理。
...
1. 用户先选择工作模式：统一管理模式由管理员在当前主机初始化用户中心，操作员导入证书配置后可注册普通标注员账号或登录；离线模式不要求账号，直接进入本机工作区。
...
真实混合记录以日期目录作为 episode 根：

```text
20260821_060700_.../
  manifest.json                 storage_format 为 hybrid-h264-jpeg-segment-v1
  cam0/cam0-00000.mp4
  cam0/cam0-00001.mp4
  t265/manifest.json            storage_format 为 jpeg-segment-v1
  t265/segments/segment-000000.bin
  t265/t265/left/index.jsonl
  t265/t265/right/index.jsonl
  t265/t265/pose/index.jsonl
```

顶层 MP4 和嵌套 segment 不是两个 episode。loader 从嵌套 BIN 建立 T265 状态/帧
索引，再把顶层 manifest 声明的相机流合并到同一个 `EpisodeSummary`；原生视频和
FFmpeg fallback 均扣除首个 T265 batch ID 后再按 manifest FPS 映射时间。

T265 segment BIN 记录可直接选择以下目录：
...
文件数量不固定，按 `segment-` 与 `.bin` 之间的十进制编号做数值排序后作为一条
连续 episode。每条 v1 记录由 48 字节小端 `DHSG` header、声明长度的 payload 和
16 字节 `DHSC` trailer 组成；流编号 4/5 为左右目 JPEG，6 为 pose JSON，三者通过
`batch_id` 对齐，状态时间使用 header 的 `capture_time_ns`。扫描建立 segment 路径、
记录 offset/size/CRC 的轻量索引，pose 全量解析；JPEG 在抽检和回放时按偏移读取并
校验 payload 与整记录 CRC32，不把所有图像载入内存。无关文件忽略，源 BIN 保持只读。
...
| FR-SRC-010 | P0 | 直接加载 segment BIN 文件夹为一条连续记录。 | 自动发现 `segment-*.bin`，按十进制编号数值排序且不写死数量；读取全部有效文件，忽略无关文件，通过偏移索引组合左右目 JPEG 与 pose，缺号清晰警告，空目录、截断、CRC/JSON/解码错误不导致 crash。 | 已实现；真实 6 段/1.385 GB 样例通过 |
| FR-SRC-011 | P0 | 从日期顶层目录或其批量父目录加载混合真实记录。 | `hybrid-h264-jpeg-segment-v1` 的顶层 MP4 与嵌套 T265 BIN 作为同一 episode，按首个 batch ID 和各流 FPS 同步；批量根排除隐藏 QC/benchmark 目录；中断 partial 数据清晰报错且不 crash；标注员手动选择可移动卷时不因 NAS 任务名不匹配而隐藏日期记录。 | 已实现；DOHC1TB 19 条批量实测通过 |
...
| `INVALID_SEGMENT_FILENAME` | warning | `segment-*.bin` 的编号部分不是非负十进制数，文件被忽略 |
| `SEGMENT_SEQUENCE_GAP` | warning | 首段不为 0 或相邻 segment 编号不连续 |
| `DUPLICATE_SEGMENT_NUMBER` | error | 多个 segment 文件映射到同一数值编号 |
| `UNSUPPORTED_SEGMENT_STREAM` | warning | BIN 记录中的流编号当前无法映射到已有数据流 |
| `SEGMENT_BATCH_ORDER` | warning | BIN 记录的 batch ID 相对之前记录倒退 |
...
| FR-VIS-002 | P0 | 提供播放、暂停、上一帧、下一帧、时间轴和速度选择。 | 支持 0.25x、0.5x、1x、2x；1x 固定表示记录的真实时间速度，任何速度下均不得量化跳过主时间轴帧。 | 已实现 |
...
| FR-VIS-010 | P0 | 回放和时间轴遵循选中裁剪范围。 | 播放从起点开始并在终点停止，画面、曲线和片段状态数保持一致；暂停或播放中定位到中间帧时必须暂停旧时钟、清除旧位置的预缓冲状态并从新位置向后重新预缓冲，用户再次点击播放后从所选帧继续，不得重置到裁剪起点。 | 已实现并测试 |
...
| FR-VIS-014 | P1 | 挂载源连续回放避免重复就绪渲染、保留有界预读且兼容回退不跳帧。 | 原生 MP4 重复上报同一帧就绪时不创建新进度状态；点击播放后让 Camera 0 独占前端预解码并连续预缓冲最多 360 个主时间轴帧，Camera 0 达到门槛后立即起播，其余兼容回退流在播放期间以约 10 FPS 追随预览，不执行大批量前端预解码且不得阻塞时间轴；暂停、步进或定位后五路恢复所选精确帧。播放时停用仅供静态检查的多路渲染完成进度回写，未变化的画面组件跳过 React 重绘；进度直接表示 Camera 0 的 0–100%，不得停在旧的 50% 阶段边界；每路预缓冲目标以该流自身实际末帧与裁剪末帧的较早者为准，避免从中后段播放时因低帧率流比总时间轴少末尾帧而永久等待；起播后时间轴不再等待每路图像逐帧回报，界面呈现节奏以 Camera 0 原始帧率为主，例如 60 Hz 状态时间轴上的 30 FPS Camera 0 每 33 ms 推进两个状态 tick，保持 1x 真实时长且避免 60 次整页渲染造成周期卡顿；渲染阻塞跨过多个期限时从当前时刻恢复稳定节奏，不快速连发追赶帧；低于主时间轴帧率的相机只在其原始视频帧变化时解码和换图，手动上一帧/下一帧仍按单状态帧移动；播放中继续有界预读，暂停、切换记录或卸载组件时清理。状态曲线静态轨迹仅在数据、指标或尺寸变化时重画，逐帧只更新播放头；原生 MP4 不可用时，FFmpeg 兼容通道允许首次加载更久，每次连续解码 240 个主时间轴帧到不超过 1280 像素宽的有界内存缓存，不写代理文件，Camera 0 仍逐帧返回且不得按 10 FPS 量化跳过中间帧。 | 已实现并测试；真实五路 MP4 样例通过 |
| FR-VIS-015 | P1 | NAS 上的逐帧 JPEG 回放使用低延迟有界缓冲。 | CIFS/NFS/SMB 和 Linux GVFS 网络源的 Camera 0 起播/中间定位门槛最多为 30 帧；四路次要画面按约 10 FPS 的实际显示步长分别预读最多 12 帧。普通 JPEG 通过回环只读 token 路由直接交给 WebView，避免 Base64 IPC，拒绝非十进制帧和 symlink；定位时废弃旧排队任务并从新位置开始，预读只在进程内存且不写源或本地缓存；本地和可移动源保持原 360 帧主画面跑道。 | 已实现并测试；真实 GVFS SMB JPEG 样例读取基线通过 |
...
| 编号 | 优先级 | 需求 | 验收标准 | 状态 |
| --- | --- | --- | --- | --- |
| FR-AUTH-001 | P0 | 首次启动选择统一管理或离线模式。 | 选择记录写入本地 `workspace-mode` 追加记录并在重启后恢复；统一管理模式可导入管理员配置、登录和退出；已有配置时登录页底部提供“重新导入用户中心配置”，用于同一 service ID 经 TLS 健康检查后迁移地址；离线模式直接进入本地工作区。 | 已实现并测试 |
...
| FR-AUTH-003 | P0 | 用户中心支持受限的标注员自助注册、当前姓名更新和管理员账号管理。 | 服务首次初始化只能在主机本机创建首个管理员；登录页自助注册只能固定创建 `operator`，拒绝角色字段并限制同一来源的注册频率，成功后立即登录；已登录标注员只能修改自己的显示名称，多个有效会话同步更新，账号名、角色及任务归属保持不变；监管账号刷新后可看到新账号或新姓名并分配任务。管理员页面仍可创建账号，管理员角色不得通过公开注册产生。 | 已实现并测试 |
...
| FR-ANN-006 | P0 | 总览完成标记与正式保存状态一致，重新进入恢复已保存裁剪。 | 仅预览、恢复草稿或只保存任务描述不显示“已标注”；当前任务标注及非空片段范围都保存成功后显示，重新载入仍按最新修订判定。切换任务不沿用旧任务片段。保留先显示只读画面的加载流程，检查及标注恢复完成后才挂载编辑器，避免旧草稿覆盖已保存范围或重复扫描 NAS 指纹；仅保存任务时允许恢复尚未保存的片段草稿。 | 已实现并通过桌面与窄窗口交互回归 |
...
| AT-025 | 管理员在当前主机初始化用户中心，标注员自助注册并修改显示名称，错误密码登录、正确登录、退出后直接调用数据 IPC | 首个管理员只能从主机本机初始化；自助注册固定为普通标注员且监管账号可见，角色注入和重复账号拒绝；改名只接受当前标注员的显示名称，其他会话和监管列表同步新姓名，稳定账号和任务不变；客户端配置固定证书和 service ID，错误密码拒绝，正确登录成功，退出后数据 IPC 返回 `AUTH_REQUIRED`；已有配置时登录页底部提供重新导入入口，同一 service ID 经 TLS 健康检查后允许原子更新局域网地址，不同 service ID 仍拒绝替换；服务端证书覆盖配置的旧私有 IP 兼容入口，使无互联网但局域网连通的旧客户端仍可登录。 |
...
- 当前主机通过 `pnpm user-center:install` 一键部署固定 `10.1.11.36:17880` HTTPS 用户中心；首个管理员只能在服务主机本机初始化，操作员账号可在客户端受限自助注册或由管理员创建。
- 客户端导入管理员生成的固定证书配置，校验 service ID 和私有 IP 后登录；同一 service ID 可通过重新导入原子更新 DHCP 变化后的地址，服务端可为迁移期旧私有 IP 签发同一 CA 下的兼容证书入口；密码不落客户端磁盘，服务不接收源数据、路径、标注或报告，已登录进程可在服务短暂断开时继续本地工作。
...
- 片段草稿保存在当前客户端本机并按账号、episode 和任务隔离；它不会改变播放或导出的裁剪范围，也不会替代范围裁剪的无障碍输入和边界控制。
...
### 14.65 `0.17.50` 监管任务分配保存反馈

- 修复仅从任务 JSON 导入的具体任务被保存前 NAS 目录门禁错误阻断的问题；已有 NAS 统计的任务继续限制分配数量不超过目录总量。
- 保存成功后按用户中心回包立即刷新账号概览和草稿状态，保存错误与成功确认固定显示在分配按钮附近。

### 14.66 `0.17.51` 用户中心任务分配协议门禁

- 用户中心健康响应声明结构化任务分配能力，客户端读取或保存任务前拒绝缺少该能力的旧服务并给出升级提示。
- 客户端逐项核对保存回包中的任务名、数量和总数，避免旧服务只保存总数后仍显示成功；管理员保存、普通账号登录回读纳入回归测试。

### 14.67 `0.17.52` 个人任务详情与紧凑标注描述

- 普通账号从账户旁的任务抽屉配置并加载本机任务目录，查看具体分配和按日期过滤的本人标注活动；用户中心只返回当前账号事件，不接收目录路径。
- 监管分配显示每个任务文件夹的数据总数，并支持按当前文件夹或单任务一键填入全部数量，减少数量误操作。
- 任务描述保持与任务和轨迹编码对齐的原生下拉框；选择自定义描述时才出现文本输入，并在 Enter 或失焦时保存。

### 14.68 `0.17.53` 标注员注册与分配确认

- 登录页新增普通标注员自助注册，注册成功即登录并同步进入监管账号列表；公开接口固定角色为 `operator`，拒绝角色注入并限制注册频率。
- 监管端保存跨账号重复任务前列出已有账号并二次确认；读取任务目录后，新勾选任务和当前搜索结果默认使用该任务文件夹全部数量。
- 已登录标注员可从顶部账号入口修改当前显示名称；服务只更新姓名并同步有效会话，监管端仍以不变账号聚合任务和审计归属。

### 14.69 `0.17.54` 任务运营驾驶舱

- 监管账户进入任务运营驾驶舱，查看今日完成、累计完成、当前分配与剩余量，并按任务类型和账号查看速度、平均耗时、最近活跃、预计完成时间、每小时趋势和最近七天完成趋势；界面显示用户中心连接状态与数据生成时间。
- 长时间没有可见进展的账号只标记为“可能停滞”，不自动判定异常。异常中心支持确认、受限备注、进入任务调整和关闭；重复分配区间属于高优先级预警。
- 用户中心分配计划新增普通/紧急/返工优先级、截止时间、进行中/暂停状态和稳定顺序；管理员可拖拽排序、调整数量、平均分配、按历史速度建议分配，并通过单次串行状态变更在两个标注员之间原子转移完整任务。所有同任务 episode 序号区间继续由服务端计算且互不重叠。
- 监管账户可从本机导入标注 JSON，补充 episode、片段和覆盖帧统计；这些本机指标不会上传用户中心。用户中心仍只从白名单审计字段聚合完成量、速度和趋势，不接收源路径、episode ID、描述、片段内容、图像、状态、报告或 hash。
- 质量结果只能由监管账户的独立复核流程创建，按任务和轨迹码记录通过/返工、错误类型、意见、复核人和时间；普通标注员没有质量自评入口。
- 日报、七日趋势和任务/账号统计可导出 JSON、CSV 或打印 HTML。导出内容只包含运营聚合与复核元数据，不包含源路径、图像、状态、片段内容、原始数据、报告或 hash。
- 普通标注员登录后按有效任务顺序自动载入首条分配数据；暂停任务不进入工作集。个人任务抽屉区分当前、下一和剩余任务，提供继续/下一任务操作；空格、左右方向键和方括号提供播放、逐帧和裁剪起止快捷键。用户中心审计上传失败时明确显示“本地未上传”，本机标注与草稿不伪装为已同步。
- 回归覆盖并发注册、并发同任务区间分配、原子任务转移、异常确认、质量复核、平均/速度建议分配数学，以及桌面宽视口下五个驾驶舱模块和无横向溢出。

### 14.70 `0.17.55` 多任务批量分配与日期截止

- 批量任务分配拆分为“整文件夹一键分配”和“按任务分配数量”两个模式；前者对多个已扫描任务目录自动使用全部视频数，后者允许多选任务并逐项填写总量。
- 两种模式均可多选标注员，并按平均或历史速度建议分别计算每个任务的不丢余数分配；每位标注员在一次批量操作中只提交一次合并后的完整队列。
- 已扫描目录的批量结果会核对未选标注员仍持有的数量，避免最终分配超过文件夹视频总数；来自任务 JSON 且没有目录统计的任务只能使用按数量模式。
- 管理员队列和批量操作的截止字段改为只选日期，保存为本地所选日期当天结束；监管区间表和标注员个人任务抽屉只显示日期，并在当日后缀显示“今天”。
- 前端 production build 与多任务分配、文件夹容量、日期截止纯逻辑回归通过；浏览器脚本已覆盖双模式、多选任务和当日提示。

### 14.71 `0.17.56` 网络源无响应恢复

- 标注员登录后的自动加载、手动扫描、记录加载、数据检查和监管任务目录读取，在进入进程内文件遍历前使用独立子进程检查源目录响应；NAS/NFS/SMB 挂载超过五秒没有响应时终止探测并返回 `SOURCE_UNRESPONSIVE`，不让 hard mount 的内核等待占用全局任务槽。
- 源目录探测失败后必须结束前端当前操作、清除忙碌状态并保留错误提示；退出登录、重新配置 NAS、导入本地数据和再次尝试均可继续使用，不要求结束桌面端进程。
- 响应探测只读取目录可达性，不修改源目录、不读取业务内容、不上传源路径或数据；本地操作错误历史将网络目录无响应单独分类。
- Rust 回归覆盖可用目录、缺失目录和到期终止的无响应探测；真实系统 hard NFS 故障路径用于恢复流程自检，源数据保持未改动。

### 14.72 `0.17.57` segment BIN 文件夹直读

- 目录选择入口直接识别数量不固定的 `segment-*.bin`，按十进制编号数值排序并将全部文件组合为同一条 T265 episode；无关文件不参与采集统计和指纹。
- 根据真实数据确认并解析 `DHSG` 48 字节小端 header、payload 与 `DHSC` 16 字节 trailer，左右目 JPEG 复用现有图像流，pose 复用现有状态模型，header 的 `batch_id` 和 `capture_time_ns` 作为同步帧号与原始纳秒时间。
- 全量建立轻量 offset 索引，不缓存 JPEG payload；抽检和回放时按偏移读取并校验 payload CRC32、整记录 CRC32 与 JPEG。缺号/非法名给出 warning，重复编号、截断、长度、magic、JSON、CRC 或解码问题返回可定位错误而不 crash。
- 回放只等待并渲染实际存在的左右目流，旧五路 JPEG 与 `h264-split-mp4-v1` 路径保持不变；三种依赖独立 JPEG 路径的导出 adapter 暂由后端明确阻断。
- 真实 `/t265/segments` 样例只读验收通过：6 个 segment、总计 1,385,226,321 字节、10,229 条 pose、左右各 10,229 帧，首尾帧和固定百分位抽检均成功解码为 848×800。

### 14.73 `0.17.58` 日期顶层混合记录兼容

- 直接选择日期开头的 `hybrid-h264-jpeg-segment-v1` 目录时，顶层 `cam0/cam1/cam2` 分段 MP4 与 `t265/segments` 左右目 JPEG/pose 被合并成同一条 episode；选择 `DOHC1TB` 批量根时仍只发现非隐藏的日期记录。
- 原生视频源新增 `startFrame`，以首个 T265 batch ID 为时间原点；逐帧 FFmpeg fallback 使用相同原点和 30 Hz 主时间轴映射，15 FPS 相机自然每两个 batch 复用一帧，旧 60 Hz `h264-split-mp4-v1` 保持原映射。
- 默认回归新增最小混合 MP4/BIN fixture，验证顶层扫描、状态加载、T265 读帧、视频路径和检查；真实批量 ignored 测试逐条读取 15 条完整混合记录的 T265 首尾帧并执行固定百分位抽检。
- `/media/descfly/DOHC1TB` 只读实测共发现 19 条日期目录：15 条完整混合记录无格式 error，1 条旧 MP4 保持发现，3 条 `jpeg-stream-v1` 仅含 `.partial` 或零长度媒体并稳定报告 error；源盘未写入任何文件。

### 14.74 `0.17.59` U 盘直读、离线局域网登录与连续回放

- 标注员选择可移动卷、本地盘或网络挂载时均只展示当前账号的任务名称和序号范围；数据源类型不再成为任务过滤绕过条件。
- 同一 service ID 的用户中心配置在新地址通过证书健康检查后可原子替换，服务证书支持受限私有 IP/`.local` 兼容别名，使无互联网但局域网连通的既有客户端仍可登录。
- 修正低帧率 MP4 流在公共时间轴上的实际末帧；中间定位会停止旧时钟、清除旧预缓冲并从新位置继续，不再等待不存在的尾帧或其他画面。
- Camera 0 通过仅绑定本机回环地址、使用进程随机 token 且支持 Range 的只读流持续解码源 MP4，不复制、不转码、不修改 U 盘；其他画面保持轻量跟随。原生通道不可用时，Camera 0 使用 240 帧后端批解码、360 时间轴帧前端预读和源 FPS 节奏的有界兼容回退。
- 播放期间停用多路完成进度回写、复用低帧率源帧、限制次要画面刷新，并将 telemetry 静态轨迹与逐帧播放头分离，减少 4K Camera 0 解码时的主线程波动。
- 快速发布门禁、真实 DOHC1TB MP4 解码、Camera 0 回环 Range 的 GStreamer 发现测试和源盘只读检查通过；Windows、macOS 与 Ubuntu 安装包仍由 tag 后 CI 和目标机验收。

### 14.75 `0.17.61` 五路原生连续回放

- 将仅供 Camera 0 使用的本机回环 Range 通道扩展到每一条已校验的清单 MP4；Camera 1、Camera 2、T265 Left 和 T265 Right 分别交给独立 WebView 视频元素持续解码，不再因次要逐帧预览约 10 FPS 的刷新限制而卡顿。
- Camera 0 的播放器、起播门禁、源 FPS 时钟和兼容回退保持不变；没有原生 MP4 的 BIN/JPEG 流继续使用有界逐帧回退，暂停或定位后仍显示所选精确帧。
- Camera 1/2 等较慢完成元数据加载的播放器在首次定位完成或进入可播放状态时自动补发播放请求，避免初次点击播放无画面、必须暂停再播放才能启动。
- 快速发布门禁、35 项播放回归和真实五路 H.264 经回环 Range 的 GStreamer 发现测试通过；源盘只读且未生成代理文件。

### 14.76 `0.17.62` 登录页用户中心配置迁移入口

- 已绑定用户中心的登录页底部新增“重新导入用户中心配置”入口；用户中心主机因 DHCP 或网络迁移更换地址后，操作员无需手工替换应用数据文件。
- 入口复用现有 Rust 配置迁移门禁：新配置必须通过固定证书 HTTPS 健康检查并返回相同 service ID，不同用户中心仍拒绝替换。
- 配置入口与工作模式、标注员注册链接共用次要操作区，并在窄诊断视口自动换行，避免挤占账号和密码输入区域。

### 14.77 `0.17.63` 运营、分配、复核与报表闭环

- 驾驶舱补齐 episode、片段、帧、平均单条耗时和当前剩余量；账号和任务行显示完成、剩余、速度、最近活跃和预计完成时间，每小时趋势标出峰值和低效时段。
- 结构化分配支持分钟级截止时间、普通/紧急/返工优先级、拖拽顺序、暂停、增减、单项或批量转移、平均或历史速度建议；批量确认页列出账号增减量、容量与互斥区间结果。管理员可以批量创建、暂停和恢复普通账号。
- 普通账号仅看到本人分配范围，自动恢复上次未完成 episode 和本机持久草稿；保存前确认任务、片段数和覆盖帧数，完成后进入下一条。审计事件使用客户端 UUID 幂等补传，断线期间明确显示“本地未上传”，且服务拒绝白名单外字段。
- 异常中心聚合登录无操作、分配未开始、个人速度下降、反复打开未保存、源断开、用户中心中断、保存失败、重复区间和数据检查警告/错误，继续支持确认、受限备注、转派调整与关闭；“可能停滞”保持建议语义。
- 独立复核记录原标注员、修订、片段、帧范围、错误类型、意见、复核人和父复核；返工结果自动创建返工优先级任务，返工后通过与原复核形成完整链路。监管端展示首次通过率、返工率、错误和账号趋势。
- 日报、周报和任务报表支持 JSON、CSV 与可打印 HTML；Rust 在管理员选择的目录创建唯一 partial、回读验证并原子发布，成功后桌面端打开输出文件所在文件夹。

### 14.78 `0.17.64` NAS JPEG 有界预读

- Linux 将 `fuse.gvfsd-fuse` 挂载识别为网络源，使文件管理器打开的 SMB 共享不再误用固定本地盘播放参数。
- NAS JPEG 的 Camera 0 起播和中间定位只等待 30 帧跑道，随后继续有界后台预读，不再先读取最多 360 个远程文件。
- Camera 1、Camera 2 和两路 T265 按现有约 10 FPS 预览步长各自预读 12 个将实际显示的帧，避免网络目录逐文件延迟导致小画面长期停住；全部缓存只在内存中，源数据不复制、不修改。
- 普通 JPEG 帧改由进程内随机 token 注册到 `127.0.0.1` 只读地址，WebView 直接取得并解码，不再逐帧生成 Base64 IPC payload；路由只接受十进制帧号并拒绝 symlink 文件，segment BIN 与 MP4 fallback 行为不变。
