...
| 文档版本 | 0.16 |
| 应用版本基线 | 0.16.0 |
| 文档状态 | 安全 Alpha，四产物 unsigned GitHub Release CD 与平台完整性门禁已定义，等待可信签名与目标机验收 |
| 发布平台 | Windows 10/11 x64；macOS 12+ arm64/x64；Ubuntu 20.04+ x86_64 Flatpak |
| 文档日期 | 2026-07-22 |
| 产品负责人 | 待指定 |
| 技术负责人 | 待指定 |
...
本文同时记录当前 `0.16.0` Alpha 已经验证的能力和正式发布前仍需完成的工作。标记为“已实现”不代表已经通过目标机发布验收；当前 GitHub Release 明确为没有可信发布者身份的 unsigned 通道，可信签名安装包、真实 SD 卡和长时数据测试仍是独立的生产门槛。
...
| D-019 | Release CD 只从 clean exact annotated tag 构建，并且必须同时完成 Windows x64、macOS arm64、macOS x64 和 Ubuntu x64 Flatpak 安装包；当前阶段允许公开显式 `UNSIGNED` 的完整集合，但标题、说明、文件名、报告和 manifest 必须一致披露。未来签名产物必须使用新版本/tag，不覆盖 unsigned 资产。 |
...
| D-024 | v0.16 起 Ubuntu 20.04+ x86_64 通过 GNOME 50 Flatpak 正式分发；Ubuntu 22.04 生成的 `.deb` 只作为 Flatpak 输入。Flatpak 仅开放显示、DRI、IPC 及 `/media`、`/run/media`、`/mnt`，不得开放 network。Ubuntu 可原生只读挂载 ext4 SD 卡。 |
...
- Ubuntu 20.04+ x86_64 目标机可通过 Flatpak 安装并从已挂载的 ext4/exFAT SD 卡完成同一离线工作流。
- 现场用户无需命令行脚本即可完成主要任务。
...
- 正式安装包支持 Windows 10/11 x64、macOS 12+ arm64、macOS 12+ x64，以及 Ubuntu 20.04+ x86_64 Flatpak。
...
### 12.3 Ubuntu Flatpak 包要求

- Ubuntu 正式支持范围为 x86_64 Ubuntu 20.04 及以上；Ubuntu 22.04 runner 只负责构建和验证，不把宿主机 WebKitGTK/glibc 依赖暴露给 20.04 用户。
- Tauri Linux 配置只生成 `.deb` 中间包，随后由 `packaging/flatpak/com.dohc.viewer.json` 重打包为 `DOHC-Viewer_<version>_UNSIGNED_ubuntu-x64.flatpak`；不能把 `.deb` 当作 20.04 正式安装入口。
...
- Ubuntu 用户通过 Flathub 安装 GNOME runtime；应用自身不联网、不写源 SD 卡，源卡必须由系统以只读方式挂载。
...
- Release 标题、说明、四个 installer 名称、verification report 和 manifest 必须显示 `UNSIGNED`；后续引入签名时恢复 Authenticode、timestamp、Developer ID、Gatekeeper、notarization 和可信 Flatpak 发布门禁。
- final job 重新核对四份 verification report 和安装器 SHA-256，生成 `release-manifest.json`、`SHA256SUMS.txt` 和 GitHub provenance。四平台集合完整后才解除 draft，已经公开的 tag 不允许覆盖。
...
- unsigned debug bundle 只证明本机构建和资源布局；unsigned GitHub Release 还必须通过四平台 CI 安装或挂载、直接启动、依赖和完整集合门禁，但两者都不能替代生产签名和目标机器验收。
...
| AT-027 | 推送 clean exact annotated release tag | CD 同时生成文件名带 `UNSIGNED` 的 Windows x64 NSIS、macOS arm64/x64 DMG 和 Ubuntu 20.04+ x86_64 Flatpak；四者均披露无可信发布者身份并通过依赖、安装或挂载、启动和 hash 检查；macOS 还通过 strict ad-hoc seal 与 synthetic-quarantine Gatekeeper 分类，Ubuntu 还通过 runtime/权限和 Xvfb 启动检查后才公开 Release |
...
- 四平台正式 Release CD、完整集合发布门禁、安装器 verification report、SHA-256 manifest 和 GitHub build provenance 工作流。
...
| GAP-009 | unsigned CD 已实现；仓库尚未配置 Windows/Apple/Flatpak 可信发布凭据和 signed release approver，也未产出四平台可信签名 Release | 配置受保护的签名环境，恢复 Authenticode/Developer ID/notarization/可信 Flatpak 发布门禁，以新版本生成完整 signed Release，并在 Win10/Win11、目标 Mac 和 Ubuntu 20.04+ 完成验收 |
| GAP-010 | Ubuntu Flatpak 已有 CI 构建和 Xvfb smoke，但尚无 Ubuntu 20.04 实机、真实 ext4 SD 卡和长时导入证据 | 在 Ubuntu 20.04/22.04 x86_64 实机挂载真实 SD 卡，完成只读导入、检查、回放、三格式导出和权限错误历史验收 |
...
- `src-tauri/tauri.linux.conf.json` 生成 Ubuntu 22.04 基线 `.deb` 中间包，资源包含 Linux x86_64 最小 LGPL FFmpeg、许可证和 provenance manifest。
...
- Release job 在 Ubuntu 22.04 生成 `DOHC-Viewer_<version>_UNSIGNED_ubuntu-x64.flatpak`，安装 bundle 后回读 runtime/permissions、应用资源 hash 和 FFmpeg encoder，并在 Xvfb + D-Bus 中执行 10 秒启动 smoke；报告加入四平台集合与最终 SHA-256 manifest。
...
| M1 多平台 Alpha | Windows x64 离线安装包、macOS arm64/x64 DMG、Ubuntu x86_64 Flatpak、reviewed FFmpeg 和基本硬件测试 | 进行中；四平台 unsigned CD 与完整性门禁已定义，目标机待验收 |
