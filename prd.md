...
本文同时记录当前 `0.2.0` Alpha 已经验证的能力和正式发布前仍需完成的工作。标记为“已实现”不代表已经通过 Windows 发布验收；Windows 安装包、真实 SD 卡和长时数据测试仍是独立的发布门槛。
...
| FR-SRC-006 | P1 | 明确显示卷类型、可移动介质状态和可用容量。 | UI 可以区分 SD 卡与普通本地目录。 | Windows 已实现 |
...
- Rust 导出入口复跑健康检查，error 硬阻断，warning 必须显式确认。
...
| GAP-005 | 每次导出在 Rust adapter 前重新运行全量健康检查；error fixture 证明 IPC 无法绕过。 |
