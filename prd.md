...
本文同时记录当前 `0.16.1` Alpha 已经验证的能力和正式发布前仍需完成的工作。标记为“已实现”不代表已经通过目标机发布验收；当前 GitHub Release 明确为没有可信发布者身份的 unsigned 通道，可信签名安装包、真实 SD 卡和长时数据测试仍是独立的生产门槛。
...
- Ubuntu 正式支持范围为 x86_64 Ubuntu 20.04 及以上；Ubuntu 24.04 runner 只负责构建和验证，不把宿主机 WebKitGTK/glibc 依赖暴露给 20.04 用户。
...
| AT-030 | 在 Ubuntu 24.04 CI 安装并运行 Ubuntu Flatpak | runtime 为 GNOME 50、App ID/desktop/metainfo 正确，仅有 SD 卡路径权限且无 network；FFmpeg/许可证/manifest hash 回读一致，应用在 Xvfb + D-Bus 中保持运行 10 秒 |
