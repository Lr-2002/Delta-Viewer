...
本文同时记录当前 `0.17.20` Alpha 已经验证的能力和正式发布前仍需完成的工作。标记为“已实现”不代表已经通过目标机发布验收；当前 GitHub Release 明确为没有可信发布者身份的 unsigned 通道，可信签名安装包、真实 SD 卡和长时数据测试仍是独立的生产门槛。
...
5. 用户选择现有任务或只输入名称创建任务，查看自动编码预览并按需要编辑任务描述；保存时由系统原子分配编号并记录当前登录用户。
...
物理 SD 卡
   |
   v
本地注册/登录 -> 选择目录 -> 自动扫描全部 episode -> 只读加载首条/按需加载其他记录
                                                                    |
                                                                    v
                                                全量结构/状态检查 + JPEG 抽检
                                                  /        \
