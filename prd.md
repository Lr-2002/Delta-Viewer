...
| D-032 | episode 根目录中的可选 SMPL/骨架 NPZ（优先 `smpl_skeleton.npz`，也接受文件名包含 `smpl` 或 `skeleton` 的 `.npz`）由 Rust 直接从只读源读取；支持常见的 `(frames,joints,XYZ)`/`(frames,XYZ,joints)` 浮点坐标数组和可选 frame ID 数组，受有界大小限制。展示层从首次加载帧附近估计髋部到头部/肩部的固定竖直方向并对齐 Three.js `Y-up`；优先用 SMPL 脚踝到脚部或 COCO 肩部到鼻尖的非对称解剖方向决定前方，以兼容镜像坐标，缺失时才回退到左右关节方向。固定变换只用于初始视图，后续帧相对该坐标系的真实转身和倾斜保持不变。解析失败只在回放右侧显示可行动错误，不阻断图像和状态回放；骨架流程不复制、不修改源 NPZ，也不进入三种导出。 |
...
| FR-VIS-012 | P1 | 在存在 SMPL/骨架数据时，用 Three.js 在五路图像右侧显示与当前 frame ID 同步的三维骨架。 | 桌面窗口中骨架面板位于图像右侧并有可交互视角，SMPL/COCO 的 Y-up 或 Z-up 源坐标默认以直立、面向观察者的方向打开；初始朝向以脚部或面部等非对称关节确定，兼容镜像坐标，后续转身如实显示；窄窗口按顺序显示在图像下方；缺少或解析失败时保留图像回放并显示明确状态。 | 已实现并通过 browser 检查 |
...
### 14.61 `0.17.46` JPEG 回放与骨架初始视角

- Camera 0 在回放网格中满填充显示；其他相机保持等比显示。
- SMPL/COCO 骨架初始加载时以接地姿态面向观察者，视角略微俯视并显示地面。
- JPEG 回放恢复严格逐帧推进；倍速改变单帧停留时间，不再通过跳帧追赶速度，连续播放保留有界预读队列。
...
### 14.63 `0.17.48` 骨架镜像坐标朝向

- 三维骨架改为以初始加载帧附近的解剖前方固定朝向：SMPL 使用脚踝到脚部，COCO 使用肩部到鼻尖；镜像或相反手性坐标不再仅因左右关节轴翻转而背向观察者。
- 对齐只在加载时执行，回放期间的人体真实转身和倾斜不被重置；新增镜像坐标和后续转身的回归测试。
