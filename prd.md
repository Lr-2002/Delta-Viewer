...
1.0.43 回放新增两个可选摄像头槽位，固定名称为 `cam3`、`cam4`。布局位于主四路画面和骨架区域之间，尺寸与次要摄像头一致；缺失时显示未接入占位，不阻塞已有数据。源目录 JPEG 或 MP4 manifest 检测到对应流后自动加入 EpisodeSummary、帧读取、原生播放和预览生成，旧五路数据契约保持兼容。
