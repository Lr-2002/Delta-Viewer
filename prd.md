...
本文同时记录当前 `0.17.26` Alpha 已经验证的能力和正式发布前仍需完成的工作。标记为“已实现”不代表已经通过目标机发布验收；当前 GitHub Release 明确为没有可信发布者身份的 unsigned 通道，可信签名安装包、真实 SD 卡和长时数据测试仍是独立的生产门槛。
...
| 编号 | 决策 |
| --- | --- |
| D-001 | 产品数据运行时只通过操作系统已挂载或映射为目录的文件系统路径读取 SD 卡、本地目录或网络文件系统目录（如 Windows 映射盘、SMB/NFS 挂载）。应用不实现 SSH、HTTP、云存储或 NAS 连接协议；网络映射盘仅作为只读源目录，不写回、不自动复制。固定局域网用户中心只承载账号登录和管理员账号管理，不承载采集数据。客户端自动更新只能访问 D-031 指定的固定 IP 镜像。 |
...
| D-008 | 核心检查、回放、标注和导出不依赖运行时网络。工作区入口提供统一管理与离线两种模式：统一管理模式连接当前主机的固定局域网用户中心登录；离线模式没有账号、用户中心请求或自动更新，直接进入本地工作区。Windows 安装包必须包含离线 WebView2 安装能力和 FFmpeg；用户中心短暂不可用不得中断已登录会话，更新失败或断网不得阻断本地功能。 |
...
| D-013 | v0.9 支持对单条轨迹做连续、闭区间的帧范围裁剪；裁剪只影响本次回放和导出，源目录保持不变。 |
...
| D-032 | episode 根目录中的可选 SMPL/骨架 NPZ（优先 `smpl_skeleton.npz`，也接受文件名包含 `smpl` 或 `skeleton` 的 `.npz`）由 Rust 直接从只读源读取；支持常见的 `(frames,joints,XYZ)`/`(frames,XYZ,joints)` 浮点坐标数组和可选 frame ID 数组，受有界大小限制。解析失败只在回放右侧显示可行动错误，不阻断图像和状态回放；不复制、不写回源目录，也不进入三种导出。 |
| D-033 | 工作模式选择是本地持久化的安全边界。未选择模式时数据 IPC 返回 `WORKSPACE_MODE_REQUIRED`；统一管理模式无会话时返回 `AUTH_REQUIRED`；离线模式允许本地数据 IPC，但用户中心和更新命令返回 `MANAGED_MODE_REQUIRED`。切换模式必须清空当前工作区状态，且不得修改源目录、发送业务数据或自动复制数据。 |
...
片段名称和注解、点击时间线跳转。切换 episode 或刷新后清空，不写源卡、不进入 Rust
永久标注，也不改变单区间导出和三个 adapter 契约。永久保存、修订审计和导出继承
必须在后续通过登录门禁后的 Rust command 和 appLocalData 原子写入实现。分段草稿
不得取代回放和导出所使用的单一连续闭区间时间裁剪；该裁剪始终提供范围滑块、数值
输入、当前帧设点和重置操作。
...
3. 应用直接从源路径只读加载首条 session，运行结构/状态检查和固定百分位 JPEG 抽检；不未经用户操作创建本机数据副本。
4. 其余 session 保持可用状态；用户从左侧双击时将该条完整原始数据复制、校验到应用本地缓存，按 Enter/空格时仍从源路径按需读取和检查。
...
| FR-SRC-005 | P0 | 源目录中的原始采集数据在整个应用流程中只读；片段标注允许一个受控 sidecar 例外。 | 不修改原始图像、`states.jsonl` 或骨架文件；用户点击“保存片段”时在当前 episode 根目录安全创建或更新应用自有 `metadata.json`，并将其排除在扫描、指纹、校验和导入数据集合之外。 | 已实现；实卡写权限仍需验收 |
| FR-SRC-006 | P1 | 明确显示卷类型、可移动介质状态和可用容量。 | UI 可以区分 SD 卡、普通本地目录和网络磁盘。 | Windows/macOS 已实现，虚拟 ExFAT 识别通过；待实机验收 |
| FR-SRC-007 | P0 | 左侧 episode 列表负责 session 选择、进入回放和显式本地缓存。 | 单击只更新选中项；双击将完整 episode 复制到应用本地缓存并执行大小/BLAKE3 校验；聚焦后按 Enter/空格从源路径读取并检查后进入回放；选中身份始终绑定源 session。 | 已实现 |
...
| FR-ANN-003 | P0 | 标注记录数据身份、处理来源和修订历史。 | 绑定规范化 episode 路径与指纹；每次保存追加不可覆盖的本机修订文件，统一管理模式记录账号，离线模式记录固定本机 provenance 标记且 UI 不显示为处理人；保存片段时同时在 episode 根目录更新应用自有 `metadata.json`。 | 已实现并测试 |
...
- 源路径只读；正常 UI 直接读取源路径且不创建数据副本。压力验收中的显式导入只写独立工作卷，导出只写用户选择的导出目录。
...
- 左侧栏：episode 列表、选中状态、文件数、容量和五路存在状态；单击选择，双击复制并校验到本地缓存，聚焦后按 Enter/空格进入回放。
...
- 页面顶部是 episode 级数据标注：可以选择或创建任务；轨迹码为系统只读预览，任务描述可编辑，并显示处理人和保存状态。
...
| AT-035 | episode 根目录放置合成 `smpl_skeleton.npz` 并加载回放 | Rust 读取坐标和 frame ID，三维骨架在 1440x920 中位于图像右侧、在 390x844 中位于图像下方；画布像素非空、当前帧切换同步、视角控件可交互；非法 NPZ 显示错误但不阻断图像回放 |
...
- 正常 UI 从源路径直接只读扫描、检查、回放和导出，不自动创建本地数据副本。
