...
本文同时记录当前 `0.15.1` Alpha 已经验证的能力和正式发布前仍需完成的工作。标记为“已实现”不代表已经通过目标机发布验收；当前 GitHub Release 明确为没有可信发布者身份的 unsigned 通道，可信签名安装包、真实 SD 卡和长时数据测试仍是独立的生产门槛。
...
| D-021 | unsigned macOS app 仍必须对 FFmpeg、主程序和完整 bundle 生成结构有效的 ad-hoc seal；发布门禁必须在 synthetic quarantine 下区分“无可信身份/未公证”的预期策略拒绝与 invalid signature/damaged 的包结构错误。 |
...
- 复制后的 app 必须添加 synthetic quarantine；`syspolicy_check distribution` 只允许因 ad-hoc identity 和 missing notary ticket 拒绝，invalid signature、missing resources、damaged 或其他结构问题一律阻止发布。用户首次启动仍需通过系统设置执行一次性“仍要打开”。
...
| AT-029 | 对 macOS Release app 添加 quarantine 并执行分发策略检查 | app/main/FFmpeg 的 nested code 与 sealed resources 严格校验通过；策略只报告 ad-hoc identity 和 missing notary ticket，不出现 invalid signature、missing resources 或 damaged |
...
- macOS arm64/x64 从固定 hash 和 commit 的 FFmpeg 8.1.2 官方源码构建最小 LGPL sidecar，拒绝 `--enable-nonfree` 和非系统动态库；headless 只读 DMG 验证 `/Applications` 链接、资源 hash 和复制后 8 秒直接启动。发布后发现该 direct-startup 检查绕过了 Gatekeeper，两个 Mac DMG 的 app resource seal 无效并可能显示“已损坏”；这些资产已由 `0.15.1` 取代，Windows 资产不受影响。
...
### 14.17 `0.15.1` macOS Gatekeeper 包结构修复
...
- 用户文档将 `0.15.0` Mac 包明确标记为已取代，并为 `0.15.1+` 提供系统设置中的一次性“仍要打开”流程；不要求关闭 Gatekeeper 或移除 quarantine。
