...
| FR-SRC-011 | P0 | 从日期顶层目录或其批量父目录加载混合真实记录。 | `hybrid-h264-jpeg-segment-v1` 的顶层 MP4 与嵌套 T265 BIN 作为同一 episode，按首个 batch ID 和各流 FPS 同步；批量根排除隐藏 QC/benchmark 目录；中断 partial 数据清晰报错且不 crash。 | 已实现；DOHC1TB 19 条批量实测通过 |
...
| AT-025 | 管理员在当前主机初始化用户中心，标注员自助注册并修改显示名称，错误密码登录、正确登录、退出后直接调用数据 IPC | 首个管理员只能从主机本机初始化；自助注册固定为普通标注员且监管账号可见，角色注入和重复账号拒绝；改名只接受当前标注员的显示名称，其他会话和监管列表同步新姓名，稳定账号和任务不变；客户端配置固定证书和 service ID，错误密码拒绝，正确登录成功，退出后数据 IPC 返回 `AUTH_REQUIRED` |
