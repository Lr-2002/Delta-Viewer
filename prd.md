...
| D-025 | v0.17 起 Ubuntu 22.04+ x86_64 首选原生 `.deb`，由 Ubuntu 22.04 runner 构建并通过 `apt` 真实安装、依赖/资源回读和 Xvfb 启动检查；下游 Ubuntu 24.04 job 只消费这份已验证 deb 构建 GNOME 50 Flatpak，继续作为 Ubuntu 20.04+ 兼容入口。两份 Linux 安装器必须同时进入 Release。 |
...
| FR-SRC-007 | P0 | 左侧 episode 列表只负责 session 选择和进入回放。 | 单击只更新选中项，不切换主工作区；双击或聚焦后按 Enter/空格未加载项时执行本地导入与检查后进入回放，已加载项时直接进入回放；导入后的选中高亮仍绑定源 session。 | 已实现并通过三视口交互检查 |
...
- Flatpak job 固定在 Ubuntu 24.04，必须下载 deb job 上传的同一份已验证 deb，再通过 `packaging/flatpak/com.dohc.viewer.json` 重打包；不得独立重建另一个 deb。该分离避免 Ubuntu 22.04 的旧 `flatpak-builder` 与 GNOME 50 AppStream 工具不兼容，同时继续隔离最终用户宿主的 WebKitGTK/glibc 差异。
...
| AT-030 | Ubuntu 24.04 CI 下载已经在 22.04 验证的 deb，并安装运行由它生成的 Flatpak | runtime 为 GNOME 50、App ID/desktop/metainfo 正确，仅有 SD 卡路径权限且无 network；FFmpeg/许可证/manifest hash 回读一致，应用在 Xvfb + D-Bus 中保持运行 10 秒 |
