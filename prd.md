...
1.0.57 主窗口关闭原生文件拖放拦截，使 Windows WebView2 接收标签库的 HTML5 拖放事件；目录仍通过原有目录选择器加载。1.0.56 安装包发布已停止，编号、排序和数字快捷键随 1.0.57 交付。

1.0.56 标签库条目按当前排列显示固定序号，只有文字区域可拖动排序；顺序按账号持久保存，旧标签顺序保留，新标签追加末尾，最多 80 条且满额不静默丢弃。选中片段后数字键按序号替换描述；若编号可作为两位数前缀，等待 650ms 连续输入，否则立即使用。普通输入框、组合键、输入法组合和其他弹窗不触发；标签库打开时描述框允许数字选取。替换仍保存草稿并撤销片段合格状态，使用及排序记录审核操作。

1.0.55 Viewer 顶部入口改为只读审核总览，直接打开工作台 `/qc.html?viewer=overview`。不再展示实时行为、审核记录、审核账号、任务中心或结果复核队列，不要求工作台账号登录、不转发账号 Cookie；保留真实统计接口、日期和审核人筛选、效率时间轴、自动刷新及 CSV 导出。Viewer 自身审核登录不变，工作台管理员页面及其服务端鉴权保持原规则；此页面模式不作为服务端权限边界。

1.0.54 补齐 Viewer 主窗口打开采集工作台所需的 Tauri 窗口创建、显示与聚焦权限，修复首次打开的 ACL 拒绝及重复点击的窗口唤起；远端工作台页面不获得本机 IPC 权限。

1.0.53 在 Viewer 顶栏提供真实采集工作台入口，保留远端工作台的统计、审核记录、行为、账号和任务中心 API；机标校对剪切按钮与 `X` 快捷键在片段边界自动定位到最近可分割帧。

1.0.50 不通过原因新增“状态异常”，与现有原因使用相同的确认、QC 保存、重新加载和监管记录流程。

1.0.48 无法读取媒体或机标的 session 仍允许明确判为不通过，加载失败页提供单条入口，批量及整文件夹共用相同后端。正常审核保存路径无法初始化时，在原有共享复核锁内重新读取根级 session.json.qc，仅未审核可写；原子保存 QC、原因、审核人、版本和 qcReviewers，保留采集字段、媒体与既有 description.json，不虚构片段或允许通过。已完成 QC/人工结果跳过，非法 QC、根级 session.json 损坏或缺失、只读目录、锁冲突均明确失败。共享账号记录支持重启及另一客户端回读，监管继续通过原审核队列上传。任务中心按同一根级 QC 计为已审核，服务器增量刷新后反映进度。

1.0.47 批量审核操作移到当前已加载目录上方，批量不通过只使用当前目录列表中勾选的数据，整文件夹不通过只扫描当前加载根目录；两者均只处理未审核 QC，已有通过、不通过和异常结果保持不变。任务中心只保留批次领取、进入、释放和进度查询。

1.0.46 任务中心允许审核员释放本人领取的批次，管理员仍可释放任意批次或转交；此规则替代原来仅管理员可释放的限制。确认后仅移除归属、记录操作历史，不修改源数据、QC 和审核进度；其他审核员可重新领取。服务端从登录会话确定操作者，在同一 SQLite 事务内检查当前归属并删除，拒绝释放他人或已转交任务；无归属时重复释放幂等。取消不提交，失败保留状态与可见错误，后台刷新不清除操作错误，重复点击不重复提交。

1.0.45 两个新增相机槽位分别固定绑定 `extension_left`、`extension_right`，标题保留目录名称，缺少任一路时保留同名占位；其他后端发现的辅助流不占用回放摄像头槽位。各路使用对应源的 JPEG/manifest 播放路径，分辨率从已加载图像/视频元数据补齐，不额外扫描全部媒体。所有相机在窄屏仍显示分辨率，长名称可换行；切换 session 后不沿用上条数据的尺寸。

1.0.43 回放新增两个可选摄像头槽位。加载 session 时按 manifest 的实际流名称，或根目录下包含数字 JPEG 帧的相机目录自动发现新增流，不要求固定使用 `cam3`、`cam4`；目录名直接作为画面标题。布局位于主四路画面和骨架区域之间，尺寸与次要摄像头一致；缺失时显示未接入占位，不阻塞已有数据。源目录 JPEG 或 MP4 manifest 检测到的安全流名自动加入 EpisodeSummary、帧读取、原生播放和预览生成，旧五路数据契约保持兼容。

1.0.42 未审核查询覆盖多层目录：以最多 4 个并发请求读取选中批次内的服务器目录索引，扁平显示所有待审核 session 的相对路径及数量；跳过无待审核记录的完整子目录，不扫描源媒体。查询失败或索引不完整显示错误，不能误报无未审核；切换批次/关闭后忽略旧请求，刷新和定时更新保留筛选。1.0.41 安装包发布已停止。

1.0.41 任务中心批次未审核筛选：每个批次提供“仅未审核”按钮，按需读取当前批次明细并只展示 QC 状态为待审核的 session；通过、不通过及异常条目隐藏，按钮可切换回完整批次列表，不读取其他批次。

1.0.40 批次领取与进入审核联动：服务确认领取归属属于当前审核员后，自动打开对应批次源目录，展示整批数据并沿用最近记录/首条自动加载逻辑，成功后关闭任务中心。本人已领取按钮显示“进入审核”，无需重复领取；共享目录断开、空目录或加载失败时保留归属、显示错误并允许重试，后台刷新领取状态不清除加载错误。失败的领取不触发导航，重复点击仅提交一次。管理员释放/转交与 QC 统计口径保持一致。

1.0.39 服务器任务统计：NAS 本机 Python 3.8+ 常驻进程每天北京时间 23:00 读取根级 session.json.qc，生成原始数据目录之外的 Delta-Viewer-TaskIndex/Delta-D1 索引。通过和不通过计入已审核，待审核与异常不计完成；目录不可读取导致该轮失败，继续提供上次成功快照及完成时间，链接目录标不完整。服务器监听普通目录中 session.json 的写入/原子替换/删除，合并变化后回读 QC、更新祖先计数，不信任客户端自报结论。inotify 溢出、目录结构变化、启动恢复触发校准；无事件时不反复扫描源目录。统计心跳、错误、开始时间与完成时间分别显示，心跳超过 90 秒提示服务未响应。

客户端通过现有挂载共享目录只读有界、SHA-256 校验的 JSON 索引；首页仅读根汇总，展开目录按需读取直接子项，每 15 秒刷新索引及领取状态，不再自动调用全量源扫描。未配置索引的目录明确提示，不静默退回慢扫描。原批次键和用户中心领取归属保持兼容。管理员 IPC 校验角色后可在服务自有目录原子写入 rebuild.request.json，请求 NAS 执行统计；普通账号仅刷新已有结果。共享目录 ACL 是文件级访问边界，部署应允许审核员读索引、限制服务脚本修改及重建信号写权限；客户端角色门禁不能替代 NAS ACL。本次用户授权允许该索引和重建信号写入独立服务目录，源媒体仍只读，既有审核 QC 写入规则不变。

服务端使用 Python 标准库，不新增客户端依赖；索引按目录分片，内容寻址、原子发布总清单，清理仅限服务 nodes 中超过一小时且当前未引用的 hash 命名旧分片。服务重启和首次部署需要扫描；定时失败 5 分钟后重试，不宣称缓存涵盖尚未发现的数据，外部修改与网络中断可能造成短暂延迟。23:00 是开始时间，顶部展示最近成功完成时间。

1.0.38 发布补充：并发浏览器夹具补齐新增监管持久化 IPC 返回值；客户端验证落盘/补传状态，空值或非法确认不删除待存记录、不导致未捕获异常。1.0.37 未发布安装包，监管队列修复随 1.0.38 交付。

1.0.37 监管队列配额修复：桌面端监管操作写入 appLocalData/review-outbox/events.sqlite（WAL、synchronous=FULL），按 service ID 与账号隔离；新记录不使用 localStorage。旧 dohc.review-audit.v1 记录每批最多 20 条迁移，数据库事务提交后才删除对应旧键，不清理其他账号、标签库或应急草稿；无法解析的旧记录原样保留并提示。只删除服务端明确确认的已发送事件 ID，异常 REVIEW_EVENT_INVALID 单条隔离在本机数据库，其他事件继续发送；手动重试包含异常记录，不改写原时间或删除失败证据。提示同时包含待传数量、持久化失败与网络/鉴权错误，兼容旧 REVIEWER_REQUIRED 过期 token，支持当前账号原地登录补传。退出登录前等待新记录落盘，磁盘失败仍保留进程内队列并阻止退出；关闭前对未落盘事件警告。此修改不能恢复旧版关闭前仅留在内存的事件，升级前不得强制结束仍有此类事件的旧进程。

客户端新增 rusqlite 0.37.0（MIT，内置 SQLite，Cargo.lock 固定依赖），无需用户安装数据库。数据库只存待传记录，确认上传后删除；离线持续积压仍消耗本机磁盘，磁盘耗尽必须显式报错。审核结果写入和源端只读约束保持不变。自动迁移与补传回归、账号隔离/幂等/异常保留、窄屏登录重试覆盖纳入验证。

1.0.36 任务中心性能：顶层目录读取后先推送批次占位，领取人查询不等待全量 QC；有界 8 工作线程在所有层级共享，批次统计完成即更新。进程内最多缓存 100,000 条成功 QC 解析结果，每次先检查普通文件、修改时间和大小，5 分钟过期；手动刷新强制重读。最近两个账号/根目录组合的完整目录快照只保留在页面进程内，重新打开立即展示并标记待更新，未复核缓存不得用于允许领取。自动扫描间隔 5 分钟，领取状态仍每 15 秒刷新；未知/未完成统计不显示百分比，异常不算审核完成。扫描经 TaskControl 管理，可取消并显示已统计数量、最近读取路径、耗时；关闭面板取消后台扫描，打开 session 前等待扫描退出以释放任务槽。OS 正在等待的 SMB 文件操作仍受系统超时约束，不承诺断网时即时取消或固定首屏时间。所有扫描只读根级 session.json QC，不解码媒体。

扫描工作池使用 Rayon 1.12.0（MIT OR Apache-2.0，Cargo.lock 固定实际版本），不使用全局无界线程池。新增回归覆盖 1000 条 QC 冷读/缓存命中、外部修改/删除、手动强制回读、子目录缓存身份、取消、首屏渐进更新、重新打开缓存和窄屏布局。本地文件系统性能测试不作为 Windows SMB 或全量 NAS 验收。

1.0.34 任务中心：替换旧个人标注任务详情，默认使用已挂载 Delta-D1 根目录，可更换并按账号记忆。目录树展示批次领取人、领取按钮、聚合审核百分比及可展开 session；仅根级 session.json.qc 的通过/不通过计入已审核，未知 QC 标异常，不读取媒体，目录无法读取时不能声称完整进度。用户中心以稳定批次键串行原子领取并持久化归属，重复请求幂等；已被他人领取不可领取，管理员可释放或转交并记录历史。默认数据集内从子目录打开仍对应同一批次键；服务不接收完整源路径、媒体或 QC 内容。

播放器自动从数据根目录同级 Delta-Viewer-Previews/<数据集名> 查找预览，继续按吞吐切换；缺少预览时静默使用原片，已存在但校验失败时显示具体校验状态。开发预览设置按钮保留在多路回放标题栏，审核员可直接使用自动路径，开发人员可以覆盖源目录和预览目录。服务端仍需先生成预览，自动配置不保证任意带宽下的原始 4K 流畅播放。

1.0.33 NAS 自适应预览：服务器从独立预览根目录生成 480p、720p、1080p HLS，客户端校验源清单、帧数、帧率、尺寸和原片指纹后，通过 HLS 吞吐估计自动选择清晰度；预览缺失或失效时回退原始 4K。原片目录保持只读，预览输出不得与原片目录互相包含。

1.0.32 产品更名：应用与 GitHub 发布展示统一使用 Delta Viewer，登录页使用“注册审核员账号”和“审核员注册”。保留 com.dohc.viewer 应用标识、可执行文件及本机存储键，避免丢失账号配置、审核草稿、待上传审计和进度。Windows 复用已注册的旧安装目录，成功安装后更新快捷方式与卸载入口；Linux 的 delta-viewer 包声明替换旧 dohc-viewer 包。新版本发布文件采用 Delta-Viewer 前缀，镜像继续校验和读取 1.0.31 及更早版本的旧名称资产。

1.0.31 校对交互：不通过原因增加轨迹不动、无效数据、任务不符、动作过快，前后端校验及 QC 保存保持一致。单击起始/结束手柄选择微调对象并高亮，不因单击改变帧边界；左右箭头每次移动所选边界一帧，选择播放头则移动播放位置。X 等同剪刀裁剪按钮，在当前帧分割有效片段；输入框、弹窗、组合键和重复按键不触发裁剪。

1.0.30 无机标人工审核：自动模式未发现机标时，根据实际 Camera 0 视频帧数初始化只读人工审核模板，打开数据不写文件。空动作列表点击加号必须创建 [0, 视频帧数) 完整片段，与当前播放位置无关，可继续切割、编辑、保存及审核。人工结果沿用 description.json、原子保存、账号关联与 QC 协议，关闭重开及更换本机草稿目录后仍可恢复；不创建伪机标文件，不以 states 条数推断视频帧数。机标文件存在但非法时仍明确报错，不静默当作缺失。

1.0.26 审核账号删除：管理员可在审核账号表确认删除普通审核账号，禁止删除管理员。服务端串行原子移除账号及登录凭据，注销该账号全部服务端会话；独立保存账号名、显示姓名、创建时间、删除时间和操作者，不保存密码，历史审核事件及统计不清除。已删除账号从管理表隐藏，但保留历史查询及“已删除”标记，禁止通过注册、管理员创建、批量创建或启用操作复用历史身份。重复删除幂等，失败保留确认窗口及重试入口。此用户授权仅增加删除能力，不自动删除任何实际账号。

1.0.25 发布补充：Windows 并发 QC 测试的临时目录名在时间戳之外加入进程内原子序号，避免低精度时钟导致夹具重名；保留并发与全部业务断言。1.0.24 标签保留，安装包未发布；状态图、独立滚动与历史审计补传修复随 1.0.25 完整集合交付。

1.0.24 审核布局与补传：状态图最低高度 220px，画布跟随容器实际尺寸并为图例和坐标轴留白，禁止绘图尺寸超过容器而裁掉轴标签。动作片段列表独立滚动并按窗口高度限制，标题与工作区边缘至少间隔 12px。旧标注审计兼容接口允许跨天历史事件补传，保留 occurredAtMs、独立记录 receivedAtMs 并按事件 ID 幂等去重；时间必须是正安全整数，最多允许超前服务端 5 分钟，禁止通过改写时间或丢弃积压记录消除错误。

1.0.23 来源与扫描补充：来源选项按当前 session 根目录及 `.session_meta` 的实际 `bailian*.json` 和人工 `description.json`/`desorption.json` 文件生成；完整相对路径用于区分同名文件。保留现有 v3/v4 结构、episode 身份、帧范围及区间约定的严格校验，源文件与应急草稿按来源隔离。管理员 QC 总览跳过已识别 session 的相机子目录，清单时长优先；旧 JPEG 仅读取状态时间戳并浅层计数 Camera 0 文件名，不对五路媒体解码或创建全量索引。时长仍标注估算，不能把状态条数当作相机帧数。导出阶段继续完整检查与回读校验。

1.0.23 Windows 导出补充：输出清单必须同步并关闭文件句柄后，才能原子发布父目录。Windows 发布任务须运行 QC 扫描/导出的原生回归；禁止用 Linux 测试通过代替 Windows 文件系统行为验证。1.0.22 安装包发布已取消，保留不可变标签。
...
1.0.21 审核监管：管理员首屏提供审核总览、实时行为、逐条审核记录和审核账号，移除原标注分配、标注抽检、导入及报表功能。记录加载/离开、前后台、播放暂停、视频定位起止帧与媒体时间、拖动耗时、控件点击、快捷键、滚动、视图/机标切换、片段选择/修改/增删恢复、标签库新增/删除/应用、保存成功/失败、通过/不通过及原因。每条事件关联当前账号和本次加载会话，只有后端保存成功才记审核结论。支持账号、日期、行为、会话过滤，事件与会话分页，以及导出全部匹配事件的 CSV。

审核事件按账号与服务隔离暂存本机，正常每 2 秒补传，管理员每 3 秒刷新；网络延迟影响实际可见时间。断网保留待传事件并提示，服务恢复后幂等补传。SQLite 历史没有 500 条截断；客户端本机存储不足时明确提示内存待传、关闭可能丢失。在线状态以最近 45 秒的有效心跳判定，历史补传不伪装在线。耗时自数据成功载入至提交成功，包含停留时间；重复提交分别计入“通过操作”，不等同于唯一数据完成量。已通过后继续编辑，逐条结论变回待审核。

此次用户授权允许向固定证书 HTTPS 用户中心上传条目名称、路径 SHA-256 标识、帧位置、标签及修改前后文本、原因和修订，覆盖历史“复核内容不上传”的限制。明细单字段最多 2000 字符，过长文本按前 2000 字符记录；不传采集媒体、密码、token 或完整源路径。服务端从 token 确认操作者，普通审核员不得查看他人监管记录，管理员不得冒充审核员上传。升级从启用后的行为开始记录，不补造旧版没有采集的交互历史。

1.0.19 发布检查补充：标准私有样例已删除，后续开发、提交、审核和发布均无需寻找或提供该样例。`check:full` 保留通用回归、Rust 检查和 Tauri debug 构建；真实样例测试仅在另行提供匹配数据时可选运行。历史版本测试读取当前应用版本，避免后续补丁发布被旧版本断言阻断。

1.0.18 汇总发布：将本地 1.0.9 至 1.0.17 的校对、账号关联、人工优先保存和目录恢复改动纳入三平台 UNSIGNED 发布流程。自动模式在 .session_meta 和根目录中优先选择 Flash 机标，显式来源保持可选；元数据目录及机标文件拒绝符号链接，异常内容明确报错。常用描述库按账号在本机保存。完整测试及平台发布门禁通过后才公开 Release；已有可信签名、物理 SD 卡及大容量验收缺口不因此关闭。

1.0.17 账号审核关联：以后端当前登录 username 为稳定身份，审核通过或不通过时同时在根 session.json 写 reviewerUsername 及 qcReviewers[username]，保留各账号最近一次审核状态、姓名、时间、原因和唯一修订；共享关联不写机器路径。本机 machine-review-accounts 按账号和规范路径持久保存同份索引，退出、重启、改名与其他账号审核均不清除本人关联。再次编辑为待审核时，已有本人关联变为待复审，首次尚未审核的普通编辑不算已审核。加载目录后异步合并本机与共享索引，目录用“我已审”标记本人记录，不能因已审核而隐藏；源媒体仍必须可访问才能回放。各账号上次选择的目录及应急草稿隔离保存，退出时清理页面账号标记；未署名旧草稿需用户确认重试后才绑定当前账号，不凭显示姓名猜测历史归属。qc 保持字符串兼容，新原因“镜头遮挡”写为“不通过：镜头遮挡”。账号审核元数据不参与采集指纹，不上传用户中心业务路径。

1.0.16 人工优先与恢复补充：以用户当前人工快照和根级 description.json 为最终审核内容，机标仅为参考；旧修订重试自动接续最新计数，机标内容或片段数量更新不能清空人工描述或恢复已删除段。保存仍校验帧范围、唯一索引和合法审核结论；冲突覆盖前在本机 machine-review-history 保留原结果，根 description 与 session qc 均写成功后才确认保存完成。共享人工结果优先于过期本机已保存草稿，应急未保存快照在重新加载与重启后继续恢复。保存失败且本机应急快照成功保存时允许切换目录、视图和重新读取；应急备份也失败时保留页面防止丢失。目录恢复独立于用户中心的任务及活动接口，并在其他启动操作完成后自动加载，避免启动时竞争任务槽而静默丢弃。此明确用户指令覆盖历史机标变化或并发修订一律拒绝的规则。

1.0.15 审核补充：不通过必须通过选项按钮选择骨架抖动、镜头污渍、动作错误、画面过曝或其他原因；其他原因展开必填文本框，最多 1000 字。qc 保持可读字符串，例如“不通过：镜头污渍”或“不通过：其他原因：视频缺帧”。原因随保存快照进入重试与应急草稿，通过或再次编辑时清空旧原因。session.json 的 revision 与审核页完整一致，为每次保存的唯一编号及相对上次保存的中文摘要；appVersion 记录软件发布版本。description.json 的公开 _human_review.segments 去除 decision、deleted，内部草稿保留恢复所需信息；workflowVersion 4 用 reviewRevision 保存并发计数并兼容旧记录。根 session 的 decision、deleted 移除；审核字段不影响采集指纹。每次软件交付补丁版本连续递增，历史版本展示对应中文摘要。时间轴按当前有效片段次序取色并画分隔线，确保相邻片段不同色。

1.0.14 审核署名补充：保留 1.0.13 的工作台布局优化。通过或不通过时根级 session.json 记录 reviewerName（当前登录账号显示姓名）和 reviewedAt（后端时间，UTC ISO 8601），与 qc 一起原子保存。后续编辑回到待审核时删除旧署名和时间。qc、reviewerName、reviewedAt 是应用管理的审核字段，不参与采集内容指纹；其他原始字段保留。

1.0.12 审核结论补充：每次复核保存仅在 episode 根目录 session.json 新增或更新小写 qc 字段，值为“待审核”“通过”“不通过”；原设备、时间、帧数及其他字段原样保留。不存在时创建只含 qc 的 JSON 对象，已有非对象或无效 JSON 时拒绝覆盖。不再写入 .session_meta/session.json。session.json 的 qc 和应用写入 partial 不参与采集指纹，其他采集字段仍参与内容指纹。本条是明确的源端 metadata 写入例外。

1.0.12 本地补丁：复核保存遇到 Linux GVFS SMB 路径时，仅使用系统中已挂载且服务器、共享名、机标内容一致的 CIFS 目录执行文件锁和原子写入；草稿身份仍使用用户原路径，无匹配挂载时保留待保存内容并明确报错。description.json 的 annotations、关联 segments 和 _human_review.segments 均只包含保留片段，本机草稿保留删除标记，workflowVersion 3 在新主机重开时从不可变原机标补回内部删除标记。删除中间段将后一段起始帧对齐前一段结束边界（界面左闭右开），后一段结束帧保持不变；Rust 保存端也执行该转换，实际 JSON 帧号同步更新。首段删除时后一段起点移到被删段起点，末段删除不延长前段，恢复删除时收回相邻片段扩展范围。

1.0.10 本地补丁：左上五路回放与骨架、左下紧凑状态图，右上时间轴、右下动作片段。点击片段自动播放该段，播放按钮选择全局播放；空格暂停/恢复当前模式，方向上下切段，左右逐帧微调最近选中的片段边界（默认结束边界），Enter 通过。文字输入不触发快捷键。新增片段在当前空白帧创建，已有片段内则分割；分帧按钮在当前帧把选中片段一分为二。每次更新分配新补丁版本并补充中文历史摘要；本轮只生成本机 Windows 包，不上传 GitHub。

1.0.8 修复：选中动作片段后，原回放播放器仅连续播放该片段，到结束帧暂停，再次播放从片段起点开始。边界编辑、切段及删除后按当前有效片段更新播放范围。暂停时仍可定位整条视频，播放时回到选中范围；原五路相机及骨架同步逻辑保持不变。

2026-09-09 最新指令（1.0.7）：回放与校对合并为默认“校对”页，沿用原回放的五路相机、三维骨架、状态数据排布和播放器。移除数据标注及创建片段区域，替换为校对时间轴与动作片段。机标视频帧与原播放器时间轴仅在界面连接处转换，不改动解码、同步、缓冲或倍速逻辑；人工修改继续实时写入对应 review JSON。数据质量警告不再要求点击确认，直接加载所有可读取画面；检查结果及导出校验保留。此要求覆盖下文历史独立校对页和警告确认流程。

2026-09-09 本地修复：监管上传失败在独立提示条中显示，连接异常每 30 秒及恢复网络时重试；会话失效允许当前账号原地重新登录并补传，保留工作区编辑状态。补传串行执行，仅移除已确认的事件，保留补传期间新增的事件。后台报告去重标识同时包含检查模式和检查结果，避免同一源指纹的新结果与旧报告冲突；原子写入、回读校验和既有报告保留规则不变。数据质量警告及错误导出门禁继续生效。

2026-09-08 最新指令：校对编辑、帧对齐与流畅度优化纳入正式版 `1.0.2`，使用 `Delta Viewer` / `com.dohc.viewer` 和正式更新通道，必须三平台完整发布。左侧主视频、起止帧下方改为人工复核，右侧保留分段条和动作列表。此前开发版先发布 Mac 的授权仅适用于开发版历史版本。

正式版 `1.0.2` 首次包含独立“校对”工作区，仅点击该页时读取机标文件；人工回放标注页仍不展示机标。已有开发版的独立配置和人工标注目录不自动合并到正式版。保留当前 UNSIGNED 分发语义及实机验收缺口。

正式版 `1.0.2` 保留开发版恢复的人工标注流程：`TRAJECTORY_STATIC` 和 `TRAJECTORY_POSITION_UNAVAILABLE` 作为轨迹警告显示，用户确认“仍要标注”后使用任务标注、裁剪与片段保存。这覆盖下文 FR-VAL-015 的自动跳过行为。

同日最新指令要求可打开的数据完整加载。新增 `.session_meta/manifest.json` 的 MP4 清单发现，分段路径仍相对 episode 根目录解析；从前 128 条状态估计实际批次帧率，并使用各流首次 availability 为 true 的批次作为首帧偏移，避免把新的 30 Hz 记录按旧 60 Hz 映射。仅未识别任何视频/图像流时阻止空记录加载。抽检、分辨率、缺少个别流等质量问题保留在检查页，不再清空工作区；主画面单帧读取失败只暂停，仍可移动到其他帧继续查看。这覆盖旧 FR-VAL/AT 中任一图像异常即阻断或卸载的规定；错误数据导出的后端门禁不变。源文件和 `.session_meta` 只读，已有根级 manifest 格式兼容保留。

本文定义 Delta Viewer 的产品边界、数据契约、用户流程、功能需求、非功能需求和发布验收标准。产品、设计、开发和测试均以本文为共同基线。
...
Flash 兼容补充：校对支持 schema v3/v4，支持原对象属性及 `T`/`value` 属性数组。来源菜单可选择原机标或 `bailian_annotation.qwen3.8-flash.json`，自动模式优先 Flash。中文动作描述优先显示，修改只回写对应语言字段，保留另一语言、任务信息和其他未改字段。工作区不展示模型流水线状态和模型提示长文，原 JSON 字段保留。Flash 独立保存本机草稿和 `bailian_annotation.qwen3.8-flash_reviewed.json`，与原机标复核互不覆盖；锁及 partial 同样不计入采集指纹。来源切换须等待保存完成，保存失败先处理本机待保存内容。

校对页同步显示 episode 已有的三维骨架，复用只读解析和交互视角。骨架跟随校对视频帧及其后端时间轴映射，连续播放、逐帧、拖动及片段定位均同步，不使用原人工裁剪范围。桌面端与视频并排，窄窗口置于视频下方；缺少骨架不占位，解析错误保留局部提示，不阻断校对。

校对读取源目录的 `bailian_annotation.json`（schema v3），以目录名匹配 `episode_results[].episode_id`。原机标始终只读；校对不进入原人工修订、“已标注”统计或用户中心。显示机标动作、模型及独立人工判定。视频、分段条和边界控件统一使用从 0 开始的视频实际帧号，显示左闭右开范围；后端读帧和骨架使用主时间轴映射，不受 states 长度或人工裁剪限制。暂停定位采用目标帧中间时间，避免浏览器浮点 PTS 导致显示上一帧。连续拖动合并未完成的定位请求，播放不逐帧重绘整个动作列表。真实未标注缺口明确显示；共享边界微调同时移动相邻片段并保证每段至少一帧。

每段只显示中文动作描述，支持统一时间轴上的起始/结束帧拖动或输入、删除与恢复；删除按钮仅在片段行悬停或聚焦时出现。人工修改立即原子写入应用管理的 `review.3.8max.json` 或 `review.3.8flash.json`，同时保留本机恢复草稿。底部整条“通过/不通过”保存质检结论并进入下一条；每次保存生成唯一版本号和前一版本号，任一后续编辑自动回到待审核。完整复制原 JSON 的所有未知/未改字段，按原帧基数和区间规则回写改动，按稳定源索引删除记录，同步关联 segments 的边界和删除；人工复核记录放入 `_human_review`，原机标始终只读。

这一用户明确要求的源端复核 JSON、固定锁和专用 partial 是 D-001/D-026 等仅允许 description.json 写入规则的新例外；不修改其他源文件。写入必须使用有界普通文件、临时文件回读、原子替换、系统文件锁和修订/hash 冲突检查；本机保存恢复日志处理 NAS 已提交而本机草稿尚未完成的中断。新复核文件不参与采集统计/指纹。无法写入时显示错误、保留本机待保存内容并支持重试，禁止伪装保存成功。

源机标规则仍支持 0/1 帧基数及闭区间；缺少声明的历史 v3 明示假设，无法可靠匹配视频时禁用定位和编辑。缺失、非法、超限或读取失败不阻断人工标注。解析限 8 MiB、2000 片段，所有读取和保存要求登录并在 Rust blocking worker 中执行，拒绝符号链接。回归覆盖视频帧/呈现画面对齐、连续播放和切段、缺口、编辑保存、合格门禁、原字段保留、删除、重入、冲突与中断恢复。
...
episode/
  ...
  states.jsonl
  description.json              可选，Delta Viewer 保存的任务描述 metadata
```
...
exFAT 上线测试至少包括：连续写入目标最长记录时长、接近满盘、突然断电、重新插拔、Win10/Win11/macOS 挂载、单文件超过 4 GB，以及 Delta Viewer 只读扫描/检查/回放/导出和源端前后 hash 对比。独立 formal stress 仍额外执行显式导入与目标回读。
...
- 原生 deb 正式支持 x86_64 Ubuntu 22.04 及以上，文件名固定为 `Delta-Viewer_<version>_UNSIGNED_ubuntu-22.04+-x64.deb`。
...
- 配置入口与工作模式、审核员注册链接共用次要操作区，并在窄诊断视口自动换行，避免挤占账号和密码输入区域。
...
- 第 14.2 节发布阻断项全部关闭。
- 自包含 validation/export 与独立 import 回归通过；不要求标准私有样例。
