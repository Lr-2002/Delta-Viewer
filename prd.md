...
每段支持描述编辑、起止帧输入/滑条微调、删除与恢复，以及“合格/不合格”。修改立即保存为本机草稿，编辑已合格内容会撤销对应合格状态。所有保留片段合格且至少保留一段时，首次生成源根目录 `bailian_annotation_reviewed.json`；之后实时原子更新。完整复制原 JSON 的所有未知/未改字段，按原帧基数和区间规则回写改动，按稳定源索引删除记录，同步关联 segments 的边界和删除；追加 `_human_review` 保存判定、原文件 hash、修订号和处理人。后续修改未再次合格时，文件中的人工判定也随之变为待复核，不得将原模型结构校验状态当成人工合格。
