...
| 文档版本 | 0.14 |
| 应用版本基线 | 0.14.0 |
| 文档状态 | 安全 Alpha，正式 CD 已定义，等待签名依赖与目标机验收 |
...
本文同时记录当前 `0.14.0` Alpha 已经验证的能力和正式发布前仍需完成的工作。标记为“已实现”不代表已经通过目标机发布验收；签名安装包、真实 SD 卡和长时数据测试仍是独立的发布门槛。
...
| D-019 | 正式 CD 只从 clean exact annotated tag 构建，并且必须同时完成 Windows x64、macOS arm64、macOS x64 的签名安装包；三平台全部通过后才允许公开 GitHub Release，不提供 unsigned fallback。 |
...
- 可分发 app/DMG 必须使用只依赖 macOS 系统库的 reviewed FFmpeg；Homebrew 动态链接版本只能标记为 local-debug。
- app 必须启用 hardened runtime，FFmpeg Mach-O 资源先使用同一 Developer ID 独立签名并保留 upstream/签名后 hash，随后 app 完成 Developer ID Application 签名和 Apple notarization/stapling；证书和 Apple 凭据不进入仓库。
- DMG 必须只读挂载验证 app、`/Applications` 链接、版本、架构和 FFmpeg 资源 hash，再将 app 复制到本地目录执行启动 smoke。
- unsigned 或 ad-hoc app/DMG 不得对外发布；GitHub hosted runner 通过后仍需在目标 Apple Silicon/Intel Mac 验收。
...
- 正式 job 绑定有 required reviewer 的 `production-release` Environment。FFmpeg URL/hash/license/build ID 与签名凭据缺失时直接失败，不回退为 unsigned 包。
- Windows 在构建前锁定 WebView2 exact Microsoft URL/SHA-256，并检查 Authenticode、可信时间戳、NSIS 内嵌 hash、silent install/startup/uninstall；macOS 检查 Developer ID、Gatekeeper、notarization、DMG 挂载和复制后启动。
...
- unsigned debug bundle 只证明构建和资源布局，不替代 Windows/macOS 正式签名、安装和目标机器验收。
...
| AT-027 | 推送 clean exact annotated release tag | CD 同时生成 signed Windows x64 NSIS、signed/notarized macOS arm64/x64 DMG；任一签名、依赖、安装/启动或 hash 检查失败时 Release 保持不存在或 draft |
...
| GAP-009 | 正式 CD 已实现，但仓库尚未配置平台签名凭据、三架构 reviewed FFmpeg 和 release approver，也未产出三平台签名 Release | 配置受保护的 `production-release` Environment，流水线生成完整三平台 Release，并在 Win10/Win11 与目标 Mac 完成验收 |
...
### 14.16 正式 Release CD 与 GitHub Wiki 基线

- tag 驱动的 CD 已定义 Windows x64 NSIS、macOS arm64 DMG 和 macOS x64 DMG 三个原生构建；Actions 使用不可变 commit，Node/pnpm/Rust 工具链固定。
- Windows verification 要求 Authenticode/时间戳、内置 offline WebView2 和 reviewed FFmpeg，并执行 silent install、8 秒启动和 silent uninstall；macOS verification 要求 Developer ID、Gatekeeper、stapled notarization、只读 DMG、资源 hash 和复制后 8 秒启动。
...
- GitHub Wiki 已初始化，并已从 `docs/wiki` 完成首次同步；当前 GitHub Environment 仍未配置任何签名/FFmpeg 变量，因此本节只表示流水线和文档实现完成，不能关闭 GAP-001/GAP-002/GAP-009，也不能宣称已有签名正式安装包。
...
| M1 多平台 Alpha | Windows x64 离线安装包、macOS arm64/x64 DMG、reviewed FFmpeg、签名与基本硬件测试 | 待完成 |
...
2. Windows x64 与 macOS arm64/x64 发布分别使用哪个 FFmpeg 分发版本、许可证组合和视频编码器？
