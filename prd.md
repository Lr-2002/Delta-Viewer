...
| AT-025 | 管理员在当前主机初始化用户中心，标注员自助注册并修改显示名称，错误密码登录、正确登录、退出后直接调用数据 IPC | 首个管理员只能从主机本机初始化；自助注册固定为普通标注员且监管账号可见，角色注入和重复账号拒绝；改名只接受当前标注员的显示名称，其他会话和监管列表同步新姓名，稳定账号和任务不变；客户端配置固定证书和 service ID，错误密码拒绝，正确登录成功，退出后数据 IPC 返回 `AUTH_REQUIRED`；同一 service ID 经 TLS 健康检查后允许原子更新局域网地址，不同 service ID 仍拒绝替换；服务端证书覆盖配置的旧私有 IP 兼容入口，使无互联网但局域网连通的旧客户端仍可登录。 |
