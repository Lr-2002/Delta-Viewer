...
本文同时记录当前 `0.6.0` Alpha 已经验证的能力和正式发布前仍需完成的工作。标记为“已实现”不代表已经通过 Windows 发布验收；Windows 安装包、真实 SD 卡和长时数据测试仍是独立的发布门槛。
...
- MCAP/HDF5/LeRobot 适配器内部回读校验；HDF5 JPEG 以 1 MiB 有界 chunk 流式写入。
...
| GAP-007 | 长时/大容量实盘性能未知；HDF5 已流式化但尚无物理 100 GB 证据 | 用至少 100 GB/100,000 文件完成扫描、导入、检查、每种导出和内存/吞吐基线记录 |
