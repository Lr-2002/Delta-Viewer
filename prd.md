...
本文同时记录当前 `0.17.25` Alpha 已经验证的能力和正式发布前仍需完成的工作。标记为“已实现”不代表已经通过目标机发布验收；当前 GitHub Release 明确为没有可信发布者身份的 unsigned 通道，可信签名安装包、真实 SD 卡和长时数据测试仍是独立的生产门槛。
...
只提供绑定当前已加载 episode 的当前会话草稿，可建立互不重叠的连续闭区间、编辑
...
| FR-ANN-003 | P0 | 标注记录数据身份、处理来源和修订历史。 | 绑定规范化 episode 路径与指纹；每次保存追加不可覆盖的修订文件，统一管理模式记录账号，离线模式记录固定本机 provenance 标记且 UI 不显示为处理人；不写 SD 卡/episode。 | 已实现并测试 |
