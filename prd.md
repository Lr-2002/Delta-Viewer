...
| D-036 | 读取流程同时识别 `h264-split-mp4-v1` 记录：根级 `manifest.json` 声明五路固定流的分段 MP4、帧率、帧数和分辨率，`states.jsonl` 使用 60 Hz `batch_id` 主时间轴并允许位姿位于可空的 `pose` 对象中。界面按主时间轴和各流清单帧率只读同步解码预览，不修改或复制 MP4；Camera 0 连续播放优先通过仅绑定 `127.0.0.1` 随机端口、支持单区间 Range 的进程内只读媒体通道交给 WebView 持续解码。该通道不是远程数据源协议，只注册后端已规范化并确认位于当前 episode 内的普通 MP4，以每次进程随机的不透明 token 寻址，不返回文件系统路径、不监听局域网、不写缓存文件；其他流保留受限 asset/逐帧兼容通道。连续播放不得对 Camera 0 逐帧启动 FFmpeg。隐藏的 recorder benchmark/QC 目录不作为 episode。首版 MP4 兼容范围仅为扫描、状态读取和回放；仍依赖逐帧 JPEG 语义的 MCAP/HDF5/LeRobot adapter 必须后端明确阻断，直至各格式完成 MP4 原生适配与回读验收。 |
