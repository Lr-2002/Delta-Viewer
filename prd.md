...
| 文档版本 | 0.15 |
| 应用版本基线 | 0.15.0 |
| 文档状态 | 安全 Alpha，unsigned GitHub Release CD 已定义，等待签名与目标机验收 |
...
本文同时记录当前 `0.15.0` Alpha 已经验证的能力和正式发布前仍需完成的工作。标记为“已实现”不代表已经通过目标机发布验收；当前 GitHub Release 明确为 unsigned，签名安装包、真实 SD 卡和长时数据测试仍是独立的生产门槛。
...
- 当前 GitHub Release 允许 unsigned DMG，但文件名、Release 和报告必须明确标记，且不得宣称 Gatekeeper、Developer ID 或 notarization 已通过。
...
- Windows 检查 DOHC 产物为 unsigned、Microsoft WebView2 签名、NSIS 内嵌 hash、silent install/startup/uninstall；macOS 检查没有 Developer ID/notarization claim、DMG 挂载、资源 hash 和复制后直接启动。
...
| AT-027 | 推送 clean exact annotated release tag | CD 同时生成文件名带 `UNSIGNED` 的 Windows x64 NSIS、macOS arm64/x64 DMG；三者均披露无可信签名并通过依赖、安装或挂载、直接启动和 hash 检查后才公开 Release |
...
- macOS arm64/x64 从固定 hash 和 commit 的 FFmpeg 8.1.2 官方源码构建最小 LGPL sidecar，拒绝 `--enable-nonfree` 和非系统动态库；headless 只读 DMG 验证 `/Applications` 链接、资源 hash 和复制后 8 秒直接启动。
...
| M1 多平台 Alpha | Windows x64 离线安装包、macOS arm64/x64 DMG、reviewed FFmpeg 和基本硬件测试 | 进行中；unsigned CD 已完成，目标机待验收 |
