...
只提供绑定当前已加载 episode 的当前会话草稿。草稿默认以完整轨迹创建片段 1，用户可在当前帧分割所在片段，形成覆盖完整轨迹且互不重叠的连续闭区间；片段名称由顺序稳定自动生成，点击片段只编辑注解并可合并删除，点击时间线跳转。切换 episode 或刷新后清空，不写源卡；保存后由
...
任务模板配置中的 `segments` 作为手动分割后可选的片段名称模板；当前格式不含帧边界，因此导入不得自动生成不准确的分割区间。界面保留模板下拉框和注解编辑器，但不提供自由输入片段名称的控件。
...
- 片段草稿按上一片段结束帧的下一帧连续起步，并以当前播放帧作为闭区间终点；片段名称默认按顺序自动生成，时间线点击片段可选择导入的名称模板、编辑注解和执行合并删除，但不提供自由输入名称的控件。
