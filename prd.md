...
| D-037 | 监管工作台保留原任务完成概览，从已扫描任务目录和已导入任务详情生成统一任务池；具体分配工作区仅在管理员点击某个操作员的分配入口后出现，并可显式关闭返回概览。管理员按操作员勾选具体任务，并为每类任务填写正整数数量；有 NAS 目录统计时，新勾选任务和当前搜索结果默认使用该任务文件夹全部数量，仍可手动减少且不得超过目录总量；仅导入任务 JSON 时默认数量为 1。服务持久化任务名到数量的精确映射，总分配量为各项之和；同一任务已分配给其他账号时，保存前必须列出冲突账号并由管理员确认是否继续。保存结果或错误必须紧邻分配控件显示并立即刷新概览。用户中心必须在健康响应中声明结构化任务分配能力，客户端在读取和保存前强制校验，并验证保存回包完整保留任务名、数量和总数，禁止旧服务产生假成功。界面展示任务完成量、文件夹总量、详情和重复分配提示，支持搜索、当前结果全选、按文件夹全量填充、单任务全量填充与清空；旧账号的数字分配值不在新界面展示，首次保存具体任务后转为结构化分配。普通账号在账户旁的任务抽屉查看本机任务路径、分配数量、起始序号和按日期过滤的本人审计记录，并可在本机选择任务根目录后加载匹配目录；路径不得上传用户中心。 |
