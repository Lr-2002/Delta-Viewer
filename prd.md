...
Flash 兼容补充：校对支持 schema v3/v4，支持原对象属性及 `T`/`value` 属性数组。来源菜单可选择原机标或 `bailian_annotation.qwen3.8-flash.json`，自动模式优先 Flash。中文动作描述优先显示，修改只回写对应语言字段，保留另一语言、任务信息和其他未改字段。模型 issues 明示为待人工复核信息。Flash 独立保存本机草稿和 `bailian_annotation.qwen3.8-flash_reviewed.json`，与原机标复核互不覆盖；锁及 partial 同样不计入采集指纹。来源切换须等待保存完成，保存失败先处理本机待保存内容。

校对读取源目录的 `bailian_annotation.json`（schema v3），以目录名匹配 `episode_results[].episode_id`。原机标始终只读；校对不进入原人工修订、“已标注”统计或用户中心。显示机标动作、模型、原结构校验状态及独立人工判定。视频、分段条和边界控件统一使用从 0 开始的视频实际帧号，显示左闭右开范围；只有后端读帧 API 使用主时间轴映射，不受 states 长度或人工裁剪限制。暂停定位采用目标帧中间时间，避免浏览器浮点 PTS 导致显示上一帧。连续拖动合并未完成的定位请求，播放不逐帧重绘整个动作列表。真实未标注缺口明确显示；共享边界微调同时移动相邻片段并保证每段至少一帧。
