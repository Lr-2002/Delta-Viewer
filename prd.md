...
| FR-ANN-006 | P0 | 总览完成标记与正式保存状态一致，重新进入恢复已保存裁剪。 | 仅预览、恢复草稿或只保存任务描述不显示“已标注”；当前任务标注及非空片段范围都保存成功后显示，重新载入仍按最新修订判定。切换任务不沿用旧任务片段。预览挂载前恢复已保存范围，旧草稿及后台检查不得覆盖该范围；仅保存任务时允许恢复尚未保存的片段草稿。 | 已实现并通过桌面与窄窗口交互回归 |
