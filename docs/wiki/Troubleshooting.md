...
先确认操作系统文件管理器能够看到卷和 episode 文件。DOHC Viewer 不包含 ext4 驱动；ext4 卡在标准 Windows/macOS 上不能直接选择。macOS 用户可按[Paragon extFS 只读教程](Paragon-extFS-macOS)挂载，必须确认 `Mount in Read-only mode` 和只读卷状态。Ubuntu 用户应把卡只读挂载在 `/media`、`/run/media` 或 `/mnt` 下；其他挂载路径不在正式 Flatpak 权限内。不要为了读取数据直接格式化仍含数据的卡。
