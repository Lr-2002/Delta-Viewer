...
`0.17.8` 及更早版本没有自动更新能力，应先从 [固定镜像](http://10.1.11.36:17879/) 手动安装 `0.17.10`。引导版显示检查或下载失败时，可以继续离线使用检查、回放、标注和导出；先确认客户端能打开 `http://10.1.11.36:17879/healthz`，再点击顶栏更新按钮重试。Ubuntu 安装阶段可能需要在系统对话框确认管理员权限。
