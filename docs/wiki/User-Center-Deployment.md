...
当前部署目标是本机主机 `10.1.11.36`，服务端口为 `17880`。用户中心只在局域网提供账号登录，不承载采集数据或标注数据；客户端仍直接读取本机挂载的源卡。
...
操作员使用管理员提供的桌面配置文件导入客户端。配置文件包含服务 ID、固定 HTTPS 地址和证书公钥，不包含账号密码或采集数据。若客户端已经绑定其他服务，必须先由管理员确认设备归属；客户端不会自动切换用户中心。
