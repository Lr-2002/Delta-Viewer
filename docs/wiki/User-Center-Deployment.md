...
新增管理员 `DELETE /api/v1/admin/users/:username`，仅可删除普通审核账号。账号及密码从 `users` 移除，全部服务端 token 失效；`users.json` 的 `deletedUsers` 保存无密码的历史身份和删除人、删除时间。审核 SQLite 和旧审计日志保留，历史账号名不可复用。升级、备份及迁移必须同时保留 `users`、`deletedUsers` 和审计文件；不要手工清理注销记录，以免历史身份混淆。

## 1.0.24 历史事件补传修复

客户端与用户中心应一并升级。旧 `/api/v1/audit/events` 曾拒绝超过 24 小时的事件，断网或周末积压会持续报 `AUDIT_TIME_INVALID`。新服务端允许历史事件幂等补传，保留原操作时间并单独记录接收时间；仍拒绝非法时间或超前服务端超过 5 分钟的时间。升级保留账号、证书、service ID 与数据目录；重启后重新登录并重试上传，不清除客户端待传队列。
