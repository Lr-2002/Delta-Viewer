...
标注只保存任务、轨迹码、统一管理处理人或离线本机 provenance、原始源路径和数据指纹，不保存 JPEG 或状态数据。离线来源不是账号，界面不会把它显示为处理人。因此原始 SD 卡或目录仍须挂载在标注时的路径；源断开的条目会显示但不能选择。后端对每条数据重新核对路径身份和指纹、执行健康检查，再使用可信检查结果调用现有 adapter。路径被替换、内容变化或存在 error 时该条失败，但后续条目继续；warning 在开始批量任务前一次确认后允许导出。

批量任务按顺序执行，避免多个大型 adapter 同时竞争磁盘和内存。取消会中止当前未完成条目并停止尚未开始的条目，已经完成回读并原子发布的输出会保留。结果列表逐条显示成功路径或失败原因；成功项可用“打开文件所在位置”在文件管理器中定位输出，每个失败项都会在应用本地数据目录的 `reports/operation-errors` 中生成独立 JSON 日志，可从结果行查看错误并定位日志文件。
