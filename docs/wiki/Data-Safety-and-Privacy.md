...
- 本地登录后先从固定公网镜像 `http://39.155.172.162:17879`，仅在内网回环不可用时从固定局域网 fallback `http://10.1.11.36:17879` 读取 `latest.json` 和当前平台更新资产；客户端不访问 GitHub，请求不附带账号、源路径、标注、报告、hash 或遥测。镜像断开或更新失败不影响核心数据工作流。
...
应用只允许 updater 模块访问固定的 `http://39.155.172.162:17879` 或 `http://10.1.11.36:17879` origin 及清单指定的精确版本目录。镜像只会按受信任的请求 Host 重写为这两个 origin 之一，任意其他 Host 始终得到公网地址。更新包按清单精确大小有界读取，允许范围为 1-64 MiB，URL、大小或签名任一不匹配都拒绝安装并保留当前版本。HTTP 不提供传输身份保证，因此 Ed25519/Minisign 验签不可关闭；能够修改传输流量的攻击者最多阻断或隐藏更新，无法构造可安装的篡改包。

镜像机是唯一访问官方 GitHub Release 的运行时组件。它每 5 分钟同步一次，先验证三个 target、文件名、大小、SHA-256 和更新签名，再把完整版本从服务自有 partial 原子切换为当前版本；失败继续提供上一完整版本。镜像对客户端只开放 GET/HEAD，不记录账号或业务数据，只保留当前版和上一版两个 release 集合，避免缓存无限增长。
