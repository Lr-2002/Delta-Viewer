...
- 本地登录后只从固定镜像 `http://10.1.11.36:17879` 读取 `latest.json` 和当前平台更新资产；客户端不访问 GitHub，请求不附带账号、源路径、标注、报告、hash 或遥测。镜像断开或更新失败不影响核心数据工作流。
...
应用只允许 updater 模块访问固定的 `http://10.1.11.36:17879` origin 和清单指定的精确版本目录。更新包按清单精确大小有界读取，允许范围为 1-64 MiB，URL、大小或签名任一不匹配都拒绝安装并保留当前版本。内网 HTTP 不提供传输身份保证，因此 Ed25519/Minisign 验签不可关闭；能够修改局域网流量的攻击者最多阻断或隐藏更新，无法构造可安装的篡改包。
