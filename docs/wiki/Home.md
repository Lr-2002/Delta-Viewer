...
当前 Release 通道明确为 `UNSIGNED`，即没有可信发布者身份：Windows 没有 Authenticode，macOS 没有 Developer ID 或 Apple notarization。macOS app 仍使用完整的本地 ad-hoc seal，并在合成 quarantine 下验证 Gatekeeper 只能因不可信身份和缺少 notarization ticket 而拒绝，不能出现资源封印损坏。Release 只有在 Windows 与两种 macOS 架构全部完成依赖校验、安装或挂载、启动 smoke 和 SHA-256 汇总后才会公开。每个 Release 同时附带 `SHA256SUMS.txt`、`release-manifest.json` 和 GitHub build provenance。
