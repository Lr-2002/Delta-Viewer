...
核心流程不需要网络，不会上传账号、路径、图像、状态、标注或 hash。`0.17.12` 起登录后只访问固定公网镜像 `39.155.172.162:17879` 与局域网地址 `10.1.11.36:17879`；发现更新会并行测速并选择更快的路径，再检查并验签安装更新。客户端不访问 GitHub，镜像或网络不可用也不影响核心流程。源 SD 卡始终作为只读数据源。
