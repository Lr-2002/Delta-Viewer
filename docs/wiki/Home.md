...
固定 IP 更新镜像 [http://10.1.11.36:17879/](http://10.1.11.36:17879/) 提供以下安装产物，用户不需要访问 GitHub：

- Windows 10/11 x64 离线 NSIS 安装器。
- macOS 12 及以上 Apple Silicon DMG。
- Ubuntu 22.04 及以上 x86_64 原生 deb（首选）。

核心流程不需要网络，不会上传账号、路径、图像、状态、标注或 hash。`0.17.10` 起登录后只访问固定镜像 `10.1.11.36:17879` 检查并验签安装更新；客户端不访问 GitHub，镜像或网络不可用也不影响核心流程。源 SD 卡始终作为只读数据源。
