...
源数据流程只处理操作系统可见的已挂载目录，包括本地目录、SD 卡和网络映射盘；应用不自行连接 SSH、HTTP、云存储或 NAS 协议，也不会上传路径、图像、状态、标注或 hash。启动时先选择工作模式：统一管理模式通过固定证书连接局域网用户中心 `10.1.11.200:17880` 登录；离线模式不需要账号，也不请求用户中心或更新镜像。统一管理模式的自动更新只访问固定公网镜像 `39.155.172.162:17879` 与局域网地址 `10.1.11.200:17879`，发现更新会并行测速并选择更快的路径，再检查并验签安装更新。客户端不访问 GitHub，镜像或用户中心不可用不会改变本地数据工作流。源采集文件只读；保存标注需要源 episode 可写，以便更新 `description.json`。
