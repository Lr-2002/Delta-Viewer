...
export function PreviewSettings() {
  ...
  return <>
    <button className="icon-button" title="预览目录设置" aria-label="预览目录设置" onClick={() => { setError(""); setOpen(true); }}><Settings2 size={17} /></button>
    {open && <div className="modal-backdrop"><section className="preview-settings" role="dialog" aria-modal="true" aria-labelledby="preview-title">
      <header><h2 id="preview-title">预览目录</h2><button className="icon-button" aria-label="关闭预览设置" disabled={busy} onClick={() => setOpen(false)}><X size={18} /></button></header>
      ...
      <footer><button className="button" disabled={busy} onClick={() => setOpen(false)}>取消</button><button className="button button-primary" disabled={busy || !location.sourceRoot || !location.previewRoot} onClick={() => void save()}>{busy ? "保存中" : "保存"}</button></footer>
    </section></div>}
  </>;
}
