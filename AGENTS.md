...
1. 源 SD 卡中的采集数据是只读数据源。不得修改、重命名或删除原始图像、`states.jsonl`、骨架文件或其他采集文件；唯一允许的受控写入是在用户点击“保存片段”后，于当前 episode 根目录创建或更新应用自有的 `metadata.json` 片段标注 sidecar。该文件必须使用 partial + rename 安全更新，并从扫描、数据统计、指纹、校验和导入数据集合中排除。
...
3. 正常工作流是“选择 SD 卡 -> 自动扫描全部 session -> 直接只读加载首条 session -> 健康检查 -> 回放/导出”。正常 UI 不自动复制源数据；用户显式双击左侧 episode 时，应用可将该条完整原始数据复制并校验到应用本地缓存。未缓存时使用期间源卷必须保持挂载。
...
- Browser demo 仅用于视觉开发，必须和真实样例统计、warning 和类型保持一致。其工作模式、账号、用户任务和标注只保存在当前页面进程内，刷新后重置；离线演示不得渲染账号/处理人或发起用户中心请求。交互抽检基线是报告 format v4、26 个已检查文件、每流 5 帧、`[1,25,50,73,99]`、30 FPS 帧率统计和非空 `autoReportPath`。它不能被当作账号安全、后端门禁或数据验收。
...
- 选择 SD 卡后自动扫描全部 session，并直接从源路径只读加载、检查第一条记录；不得未经用户操作自动创建本地数据副本，也不显示导入目标。左侧 episode 列表以源路径作为稳定身份：单击只选择，双击将完整 episode 复制并校验到应用本地缓存，聚焦后按 Enter/空格才从源读取并进入回放；异步操作成功或失败后焦点必须回到触发的 session。失败和权限错误必须写入本地操作历史。
