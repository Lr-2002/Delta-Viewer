...
2. 数据运行时只处理本机目录，不增加 SSH、HTTP、云存储或遥测数据路径。统一管理模式中，用户中心是唯一的业务身份例外：客户端只通过管理员导入的固定证书访问当前主机局域网 HTTPS 服务，不发送源路径、图像、状态、标注、报告或 hash；自动更新仍只按顺序读取固定公网镜像 `http://39.155.172.162:17879` 与固定局域网 fallback `http://10.1.11.36:17879`。离线模式不得连接用户中心或更新镜像。
